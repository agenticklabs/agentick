...
> **v2 (`feat/v2`) — the /testing doubles law.** v2 test doubles follow the Meszaros taxonomy: `fake*` (minimal working impl, the default), `stub*` (canned answers), `spy*` (call recorders), `mock*` (expectations). Never `test*` / `createTest*`. Each layer ships its doubles under a `/testing` subpath typed against the spec (e.g. `@agentick/knobs/testing`, `@agentick/compiler-react/testing`) — there is no `@agentick/core/testing` in v2. Grep the target package's `src/` and `@agentick/utils` (+ its `/testing`) for an existing helper before writing one. Tests run via `npx vitest run packages` from the repo root.
>
