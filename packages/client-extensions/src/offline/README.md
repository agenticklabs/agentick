# `@agentick/client-extensions/offline`

Offline-queue extension for `@agentick/client-core`. Buffers outbound
...
```ts
import type { OfflineStore, QueuedRequest } from "@agentick/client-extensions/offline";
...
- **IndexedDB store** — adopter-supplied `OfflineStore` impl. We may
  ship a sibling package (e.g., `@agentick/client-offline-indexeddb`)
