...
export interface McpServerToolsConfig {
   ...
   * `createToolCatalog` from `@agentick/tool`). Arrays wrap
...
export interface McpServerOptions {
   ...
   * the `complete*` sugar builders from `@agentick/mcp`.
   */
 ...
 * `isPromptsInstance` from `@agentick/spec` (the canonical
 ...
 * `isResourcesInstance` from `@agentick/spec` (the canonical
 * structural guard).
 */
...
export function resolveToolsOption(option: McpServerToolsOptions): ResolvedToolsOptions {
  ...
  if (Array.isArray(cfg.registry)) {
    ...
    throw invalid(
      "tools.registry must be a ToolDeclaration[] or a ToolCatalog (from @agentick/tool)",
      ["tools", "registry"],
    );
  }
  return {
