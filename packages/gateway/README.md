...
**A conversation list is a predicate, not a flag.** `{ internal: false, anchored: false }` is the list a person sees (ADR 100 law 2): internal sessions — the plumbing a spawned worker runs in — render nowhere, and an anchored session renders under the entry it hangs from rather than beside it. A FORK of a conversation is still a conversation, which is why it is a composed predicate and not a `root` flag. The other question, `{ fromSessionId }`, asks for everything branched from one session — one level of the graph, forks and replies and workers alike.
...
The same door reads ownership in the other direction too. `app/create_session` takes a **`from`** bag — the one edge a fork, a reply and a spawned worker all ride (ADR 100) — and admits it only when the caller owns the SOURCE session, because `from.inherited` fans that session's timeline, knobs and state into the new one. `internal` is not on the wire at all: like `principal`, the edge declares it.

**A branch names its source, not its app.** With a `from`, `appId` is optional — the gateway resolves the app from the source session, because a branch lives where the conversation it came from lives; name one anyway and it is checked against the resolved app rather than trusted. Every miss is the same refusal: an unowned source, a source that does not exist, and an `appId` that is not the source's app are indistinguishable, so the door is never an oracle for which sessions exist or which app hosts them. `gateway.as(identity)` runs the identical guard — a door that carries an identity carries the rule that identity is for.
...
A signal joins the stream when its execution **is** this turn, or when the emitting session's lineage reaches this turn — the child, the grandchild, and work a descendant started from a later turn of its own. Membership is the same origin edge [`abortExecutionTree`](../app#the-cancellation-ladder) fans out over, read from the other end: `app.executionTreeContains(executionId, sessionId)` climbs the live parent chain and stops at the first ancestor stamped with that origin execution.
...
| `attachScopeNode(path)`          | Hold one node of the shared tree open for an attachment.         |
...
- `src/__tests__/create-session-from-guard.spec.ts` — ADR 100 law 4 at both create doors: a `from` naming another principal's session refused with nothing created, the source's own owner admitted, an absent source refused with the identical error, an unstamped source open to anyone (the `visibleTo` rule), and a create with no `from` untouched. Plus the address half — a branch with no `appId` landing in the source's app and only there, a mismatched `appId` refused with the same error an absent source gets, an agreeing one admitted, and a plain create still needing an app to name — and the `gateway.as(identity)` door refusing and admitting exactly as the wire does.
