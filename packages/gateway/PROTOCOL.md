...
Call `schema` with no params to discover the full protocol.
...
`schema`, `tool-catalog`, `tool-confirm`, `tool-dispatch`.

Custom methods are registered via `methods` config or plugin `registerMethod`.
