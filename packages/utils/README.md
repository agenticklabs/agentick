...
`@agentick/utils` is the **lowest** layer of the v2 dependency graph:
...
`LocalPubSub`) live in
[`@agentick/pubsub`](../pubsub) — they belong with the
...
```ts
import { isPlainObject, isEqual, mergeLayered, append } from "@agentick/utils";
...
## Loaders subpath — `@agentick/utils/loaders` + `/loaders/node`
...
## Testing subpath — `@agentick/utils/testing`
...
- Anything that imports from `@agentick/spec`, a harness, or a
...
[`@agentick/pubsub`](../pubsub) — see that package's "Verified by"
...
- No `effect`-flavored variants — adopters needing `Equal.symbol`
  ...
  directly.
- No streaming/iterator helpers — those belong in `@agentick/runtime`
