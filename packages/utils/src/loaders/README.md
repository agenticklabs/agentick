...
- `@agentick/utils/loaders` — platform-agnostic primitives
- `@agentick/utils/loaders/node` — Node `fs`-backed sources

Each harness package (`@agentick/skills`, `@agentick/prompts`, future `@agentick/resources`) composes these primitives into its OWN public `fromArray / fromFile / fromDirectory / ...` API. The set of sources that's _sound_ for a record type depends on whether the record carries unserializable code — that constraint is harness-specific, so this module deliberately does NOT ship a unified `from*` surface.
...
## Primitives — `@agentick/utils/loaders`
...
## Node primitives — `@agentick/utils/loaders/node`
...
```ts
// in @agentick/skills
import { mapLoader, sourceFromArray, extractFrontmatter } from "@agentick/utils/loaders";
...
Same skeleton in `@agentick/prompts`, except its `fromDirectory` is omitted (JSX `.tsx` files on disk need a bundler), and `fromModule` exists so React-authored prompts compose.

## Composing sources

```ts
import { mergeLoaders } from "@agentick/utils/loaders";
