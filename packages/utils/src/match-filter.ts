 ...
 * `@agentick/cluster`'s `AddressFilter`; declared here so the
 ...
 * `@agentick/cluster`'s `EventFilter`.
 */
 ...
 * `@agentick/runtime`'s `matchesQuery` against the full
 * `EventQuery` shape.
 *
 * TODO(phase-5): move `matchesQuery` + `compileQuery` from
 * `@agentick/runtime` into this package alongside the filter
 ...
 * substrate. It backs:
 *   - `matchesQuery` / `compileQuery`'s `scope` clause (`@agentick/runtime`),
 *   - every `CollectionStore`'s scope-filtered `list` (e.g. the tasks store).
 *
 * Structural + generic by design so `@agentick/utils` stays a leaf: the
 ...
 *
 * @verifiedBy packages/utils/src/__tests__/match-scope.spec.ts
 */
 ...
 * scope clause on the publish loop (`@agentick/runtime`) builds one matcher
