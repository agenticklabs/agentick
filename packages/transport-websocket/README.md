...
- `src/__tests__/subscription-first-frame.spec.ts` — over a real socket, a subscriber attaching after a session's knobs already exist receives the `knobs-state` snapshot as its first frame. WS is where the old synchronous-re-key defence was aimed (one ordered channel delivering `[response, event, event]` in a single tick), so it is where the client-allocated id replacing it has to be shown not to cost anything.
