...
- `src/__tests__/cache-hints.spec.ts` — message-level cache hints carried, a block-level hint reaching the part it rides, and a message-level hint marking the last part it covers. Plus the adjacent-text join: two sections producing the exact bytes the formatter's merge produced before it moved here, the refusal across a cache hint and across per-part provider knobs in either direction, a media part breaking the run, and a message with nothing to join coming back untouched.
