...
- **Adjacent TEXT parts of one message join with a blank line.** A provider is free to concatenate a message's text parts with no separator of its own, so two blocks arrive welded together — `# A\nfirst` running straight into `# B\nsecond`. `joinTextParts` puts the separator in at the point the parts meet, which is also what the string exit has always done (`blocksToText` joins blocks with `\n\n`). The join **stops at a boundary**: a part carrying `cache` marks a position in the prompt text, and a part carrying `providerOptions` / `providerMetadata` carries knobs scoped to itself, so such a part neither absorbs nor is absorbed. Blocks are still individually attributable in the IR; they are simply not separately addressable at the wire.

The parts are exported individually so a custom projection stays aligned with the canonical one: `buildMessages`, `buildTools`, `buildProviderTools`, `buildParameters`, `messagePartFromBlock`, `joinTextParts`, `textRuns`.
...
### Cache boundaries — what the target can honour

`applyCacheSupport(messages, target)` is the pure screen behind the executor's cache step. It reads every `CacheBoundary` a message or part carries against `target.capabilities.cache`: a target without `explicit` support declines them all — it caches on its own terms, so a marker has nothing to become — and a target with it declines a longer lifetime placed after a shorter one and keeps only the last `maxBoundaries`. The result is the messages with the declined boundaries stripped, plus a `declined` list naming each by message and part index with its reason. Nothing throws: a lost boundary costs cache efficiency, never the request. When nothing is declared the messages come back by identity.
...
| `applyCacheSupport`                                                                          | Screen cache boundaries against the target's record      |
...
- `src/__tests__/cache-support.spec.ts` — messages returned by identity when nothing is declared, whatever the target; every boundary declined and stripped from the wire on a target that caches on its own terms; a well-formed set (longer lifetimes first, within the limit) untouched; a longer lifetime after a shorter one declined; and the boundaries nearest the end kept when more are declared than the target takes.
- `src/__tests__/provenance.spec.ts` — the alignment invariant (`provenance[i][j]` describes `messages[i].content[j]`) over the trees a divergent walk would go off by one on: cache-hinted section blocks keeping one part each, several system entries merging into one message, a system entry contributing no parts, no system entry at all, a free-floating grounding message, mixed blocks, empty content; plus the timeline message id rather than a position, the section's stable id for a system part, two adjacent sections each attributable once a boundary makes them two parts, a joined part named by the block it starts at, `entryId` omitted rather than invented when there is none, and out-of-range lookups returning `undefined`.
...
- `src/__tests__/cache-hints.spec.ts` — message-level cache boundaries carried, a block-level boundary reaching the part it rides, and a message-level one marking the last part it covers. Plus the adjacent-text join: two sections producing the exact bytes the formatter's merge produced before it moved here, the refusal across a cache boundary and across per-part provider knobs in either direction, a media part breaking the run, and a message with nothing to join coming back untouched.
