...
The executor (`@agentick/model-executor`) is the harness — orchestration,
...
```ts
import { generateObject } from "@agentick/model";
import { z } from "zod";
...
`SendInput.responseFormat` directive landed in `@agentick/session`).
...
`TOOL_NARRATION_FIELD` (`"_summary"`, from `@agentick/spec`) as an
...
The tool executor (`@agentick/tool-executor`) **strips `_summary` before
...
`@agentick/spec`): `patch` wins per provider-namespace key,
...
```ts
import { effectiveModelInfo, contextUtilization, estimateTokens } from "@agentick/model";
...
| `@agentick/model-ai-sdk`    | `aisdk(model, options?)`      |

None of them depend on `@agentick/model-executor` (or Effect) at
