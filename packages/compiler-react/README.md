...
Reference implementation of the `CompilerProtocol` from `@agentick/spec`.
...
`@agentick/compiler`. It owns three things adopters touch:
...
```tsx
import { createApp } from "@agentick/app/react"; // defaults compiler to reactCompiler()
import { System, User, Section, useOnToolEnd } from "@agentick/compiler-react";
...
`@agentick/compiler` (compiler-general); the `timeline` default is
supplied here, reading `HookBridges.timeline` structurally (no dependency
on `@agentick/timeline`, per ADR 27).
...
guarantees chain lives in `@agentick/session`'s README). `schema`
...
Event shapes (from `@agentick/spec`): `LifecycleToolEnd` carries
...
Extends `@agentick/tool`'s factory with a render-time `use()` slot
...
Use with `fakeBridges()` from `@agentick/compiler` to drive the
harness in tests.
...
```tsx
import { System, useOnExecutionStart } from "@agentick/compiler-react";
...
`@agentick/state/react`'s `useSessionState` if you need it to
survive hibernate/resume.

```tsx
import { useState } from "react";
import { Section, useOnToolStart, useOnToolEnd } from "@agentick/compiler-react";
...
```ts
import { compileTemplate, renderTemplate } from "@agentick/compiler-react";
...
`@agentick/formatters` — see that package's README for how
...
- **`ExecutionTarget.capabilities` is provisional.** The capability set
  in `@agentick/spec` is synthesized from v1 and marked
...
- Tree-side `guard`/`transform` registration + unmount + per-mount isolation —
  ...
  in `@agentick/session`'s `tree-interceptors.spec.tsx`)
