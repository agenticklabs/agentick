...
async function makeHarness() {
  ...
  const harness = new CompilerHarness("h_1", journal, bus, inbox);
  await harness.ready;
  return { harness, journal, bus, inbox };
}

describe("CompilerHarness — end-to-end", () => {
  ...
  it("emits requested + terminal events into the journal", async () => {
    ...
    expect(
      journaled.some((e) => e.name === "compiler:command:mount" && e.phase === "requested"),
    ).toBe(true);
    expect(
      journaled.some(
        (e) =>
          e.name === "compiler:command:render-tree" &&
          e.phase === "terminal" &&
          e.outcome === "succeeded",
      ),
    ).toBe(true);
