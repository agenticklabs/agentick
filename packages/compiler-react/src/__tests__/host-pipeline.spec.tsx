...
describe("host-pipeline — react-reconciler end-to-end", () => {
  it("mounts a flat tree of intrinsics", () => {
    ...
    const compiler = createCompiler({ container, idPrefix: "m_1" });
  ...
  it("renders function components and propagates props", () => {
    ...
    const compiler = createCompiler({ container, idPrefix: "m_2" });
    const root = compiler.createRoot();

    function Greeting({ name }: { name: string }) {
      return React.createElement("message", { role: "user" }, `Hello, ${name}`);
    }
    compiler.render(React.createElement(Greeting, { name: "Ryan" }), root);
  ...
  it("propagates external setState through React's scheduler with flush", async () => {
    ...
    const compiler = createCompiler({ container, idPrefix: "m_state" });
    const root = compiler.createRoot();
    ...
    compiler.render(React.createElement(Counter), root);
    await flush();
  ...
  it("commits updates when props change between renders", () => {
    ...
    const compiler = createCompiler({ container, idPrefix: "m_3" });
    const root = compiler.createRoot();

    function View({ value }: { value: number }) {
      return React.createElement("section", { id: "s.count" }, `value=${value}`);
    }
    compiler.render(React.createElement(View, { value: 0 }), root);
    ...
    compiler.render(React.createElement(View, { value: 42 }), root);
    expect(textOf(container.children[0])).toBe("value=42");
  });

  it("removes children when count shrinks across renders", () => {
    const container = createContainer({ mountId: "m_4" });
    const compiler = createCompiler({ container, idPrefix: "m_4" });
    const root = compiler.createRoot();
    ...
    compiler.render(React.createElement(List, { items: ["a", "b", "c"] }), root);
    const idsOf = () =>
      container.children
    ...
    compiler.render(React.createElement(List, { items: ["a"] }), root);
  ...
  it("reorders keyed children without duplicating (within a parent element)", () => {
    ...
    const compiler = createCompiler({ container, idPrefix: "m_reorder" });
    const root = compiler.createRoot();
    ...
    compiler.render(React.createElement(List, { items: ["c", "a"] }), root);
    expect(idsOf()).toEqual(["c", "a"]);
    expect(section.children).toHaveLength(2);

    compiler.render(React.createElement(List, { items: ["b", "c", "a"] }), root);
  ...
  it("preserves component identity (hostId) across re-renders with same key", () => {
    ...
    const compiler = createCompiler({ container, idPrefix: "m_5" });
    const root = compiler.createRoot();

    function App({ label }: { label: string }) {
      return React.createElement("message", { key: "stable", role: "user" }, label);
    }
    compiler.render(React.createElement(App, { label: "hi" }), root);
    const firstId = container.children[0]!.hostId;

    compiler.render(React.createElement(App, { label: "hello" }), root);
