/**
 * `@agentick/model-openai` — OpenAI `LanguageModelAdapter` for
 * Agentick v2 (ADR 52).
 *
 * Ships `openai(model?, options?)`, a factory producing the
 * provider-normalization part consumed by the ONE
 * `LanguageModelExecutor` in `@agentick/model-executor`. Zero Effect,
...
// StreamTagParser lives in @agentick/model — re-export here so
// adopters importing from this package keep working without a deep
// import.
export { StreamTagParser } from "@agentick/model";
