 ...
 *
 * The reconciler harness wraps the user's element in
...
/**
 * Provider. Used by the reconciler harness when wrapping the user's
...
export function useBridges(): HookBridges {
  ...
  if (!bridges) {
    throw new Error(
      "useBridges (or a hook that depends on it) called outside a ReconcilerHarness mount. " +
        "Components must render inside <BridgeProvider> — the reconciler harness wraps " +
