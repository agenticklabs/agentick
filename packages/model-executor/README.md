...
[`@agentick/model`](../model/README.md).
...
- `LanguageModelExecutor<TRaw, TChunk>` — the harness. Protocol surface
  per `@agentick/spec`: `project` / `execute` / `executeStream` /
...
themselves live in `@agentick/model` as executable values
...
- The `runExecutorConformance` "command-ified model call (ADR 89 §1)"
  block (in `@agentick/spec-conformance`) — asserts `model:generate`
