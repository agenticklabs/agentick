...
The same holds for failure. A classified provider error — from the generate step or from an adapter that refuses a response at `normalize` — becomes `{ outcome: "failed", error }` with its class intact, and an abort becomes `canceled`; only a **local** defect (projection, normalization) still rejects, because re-issuing an identical request would reproduce it exactly. That split is what lets a `stream: false` deployment reach the session's tick-failure policy at all: a rejection never reaches the decide fold, so before it there was no retry off the streaming path.
...
`seenRuns` records at the `project` command, which BOTH the streaming and non-streaming paths cross — so asserting on what reached the model does not force the loop off the path production takes.

## The round-trip recorder — moved

`roundTripRecorder` now ships from **`@agentick/session`**. It grew from a
model-only capture into the full ⓪–⑤ span (rendered tree → compiled input →
native request → raw provider chunks → canonical deltas → what the timeline
persists), and that span crosses three harnesses.

It had to move: `CommandRegistry` is spec-SEEDED but harness-AUGMENTED, so a
package can only NAME hook keys whose augmenting module is in its compilation.
This package sees `model:*` and nothing else — it cannot write
`onAfterCompilerRenderTree` at all. `@agentick/session` depends on every harness
it integrates, which is why cross-harness work belongs there.

See `packages/session/README.md` and `docs/proposals/v2/observability.md`.
...
**Certifying an adapter.** `runExecutorConformance` in [@agentick/spec-conformance](../spec-conformance) runs the whole protocol contract, including the command-ified model call. Every provider package re-runs it against its own adapter. Its required second argument, `errorFixtures`, is total over `ExecuteError["_tag"]`: provider-native errors thrown from your stub client that must classify to each failure class, or `"not-applicable"`.
