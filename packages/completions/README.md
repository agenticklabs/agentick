...
- **Completion on MCP-origin prompts is not forwarded.** Prompts discovered on a remote MCP server live on the MCP client harness rather than folding into the native prompts surface, so their arguments do not complete through this seam. The shape is settled — a forwarding resolver whose body re-asks the origin server — and it waits on that fold (`TODO(mcp-prompts-fold)`). The client verb it will call already returns the origin's `total` / `hasMore`, so a forwarded answer will not misreport a truncated list as complete.
