...
- `@agentick/spec` — index; re-exports everything (`data`,
...
```ts
// in @agentick/timeline/src/augment.ts
...
`@agentick/subscriptions`) add optional slots the same way; the
...
`@agentick/session`. The same moments also fan out as
...
(`@agentick/runtime`), the streaming sibling of `runHarnessProtocol`.
