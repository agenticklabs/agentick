/**
 * Compiler snapshot and diagnostics.
 *
 * `CompilerSnapshot` is the harness-private state needed to rehydrate
 ...
 * compiler iterates `Object.entries(bridges)` and feature-tests each
 * slot for the `SnapshotCapable` contract — no harness-specific
 * knowledge lives in the compiler. New harness packages register
 ...
 * represent compiler-internal concerns that incidentally LIVE in
...
export interface CompilerSnapshot {
   ...
   *
   * Adding a new harness with snapshot support requires zero compiler
