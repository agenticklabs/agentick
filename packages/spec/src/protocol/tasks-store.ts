 ...
 * import("@agentick/tasks").InMemoryTaskStore}) and executor
 ...
 * `@agentick/tasks`), exactly like the timeline stores.
 *
 * Bundled default: `InMemoryTaskStore` (`@agentick/tasks`). A
 * `@agentick/tasks-store-postgres` conforms to this SAME protocol later.
 *
 ...
 * `@agentick/elicitation` — the sugar (schema construction, throw-
 * on-decline, `try*` variants) lives in the elicitation package and is
 * passed in. `buildElicitSugar` from `@agentick/elicitation` has
 * exactly this shape.
 *
