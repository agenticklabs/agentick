...
**A branch is not a subordinate.** `from` records where a session came from and confers no lifecycle relationship. A fork or a reply arrives at the create door with no parent edge at all, so it registers as an ordinary top-level session owned by the same principal: no ancestry (`spawnPath`), no place in the spawn-depth ceiling, no construction signal fanned into it, and no membership in its source's live tree. Every teardown that reaches a subordinate — the cancellation ladder, `abortExecutionTree`, `destroySession`, a parent close — walks that edge, so all of them stop at the branch. Closing a conversation leaves its forks running. Only a spawned worker is owned by its parent.
...
`session.close()` is the gentle one — hang up. The session ends, its durable record survives as history on a `closed` status, and its **detached** tasks keep running: they were spawned to outlive the conversation. `closed` is an _ending_, and it is terminal for the resume door — so it is reserved for a caller who meant it. A page-out, whether by the reaper or by [`closeApp`](#bounding-the-live-registry), records `hibernated` instead.
...
**`closeApp()` hibernates too.** Leaving memory is not a lifecycle end, and a process going down is leaving memory — so the shutdown cascade pages its registry out through the same snapshot-then-close eviction runs, with `reason: "shutdown"`. Every mounted session comes back on the next process. Only explicit intent — `closeSession`, `session.close()`, a `runOnce` teardown — stamps `closed`, because `closed` is terminal for the resume door: a deploy that stamped it would silently end every thread it happened to be hosting.

Shutdown parts company with eviction on refusal. The reaper declines a session with work in flight; a departing process has nowhere to stand still, so it **aborts** the execution, waits for it to settle, and checkpoints anyway. Draining the abort before the flush is what keeps that honest: the pending tool call is answered with an error result, the turn gets its `aborted` boundary, and the record hibernates with no `currentExecutionId`. The next open therefore finds a _finished_ turn, not a crash — `onInterruptedExecution` stays quiet, and the user sees an aborted turn they can send past. A process that is killed rather than closed skips all of this and leaves the `running` record the [interruption path](#a-turn-a-crash-interrupted) exists for.
...
Every eviction — LRU, idle sweep, or `evictSession` — runs `session:snapshot` then `session.close({ reason: "evicted" })`, the same operations an explicit checkpoint and teardown run, not a path around them. Shutdown runs the same pair under `reason: "shutdown"`, so the write-behind cells (knobs, state) land on the way down rather than only the timeline flush a bare close carries. So an `onBeforeSessionClose` observer sees evictions (and a guard on it, checking `reason === "evicted"`, is how you pin a session in memory), and the audit trail tells an eviction from a hangup by the record's `reason`, not by which code path ran. A flush that fails aborts the eviction and leaves the session live: an unmount behind an un-flushed tail would lose data.
...
Close emits `session:command:close` carrying its `reason` (`"closed"`, `"evicted"`, `"shutdown"`). It is bus-only by policy — the envelope reaches `app.events(...)` without filling the journal — and a veto leaves the session usable.
...
- **`onBeforeAppResumeSession` counts attempts, not resumes.** The wire's resolution walks every app asking "can you resume this id?", so the before-hook fires once per app asked — including apps that answer nothing, and ids that resume nowhere. The truthy signal is the around form: `onAppResumeSession` observing a session out of `next()` is a resume that actually happened. Telemetry and dashboards key on the around form; the before form is only right when attempts are the thing being measured. Concurrent resumes of one id (a cold open's history page + catalog polls arriving together) JOIN one op, so the around form fires once per rebuild, not once per asker. That join is process-local: cross-node double-mount is governed by session affinity — one session has one home at a time (the single-home assumption) — not by the framework.
...
- `src/__tests__/session-substrate-factory.spec.tsx` — the per-session substrate slots; the `from` bag threading through the create door onto the record while minting no live parent edge; the door reading an absent parent edge as an ordinary top-level session, outside its source's live tree where a worker sits inside it; and subordination keying on the disposition rather than the field, so a visible child passed a parent id still gets none.
- `src/__tests__/fork.spec.tsx` — `session.fork()` as the conversation verb: an unbound child carrying the source's knob values and messages through the anchor, no durable row until it speaks, then a record carrying the branch edge and no spawn ancestry, divergence in both directions afterward, and the fork outliving a close of its source that takes a spawned worker down.
...
- `src/__tests__/session-eviction.spec.tsx` — `maxActive` evicting the least-recently-active session (LRU order proven via a send that refreshes an older one), `idleTimeout` evicting a quiet session on the sweep, an evicted session reopening with its timeline rehydrated, an in-flight execution never being evicted, an evict→resume reading exactly what a fresh app over the same stores reads, no field of the app retaining the evicted session, `evictSession` as the manual caller of all of it, and — where shutdown parts company with eviction — `closeApp` over a session held mid-tool-call completing rather than refusing, the abort landing an error tool result plus an `aborted` boundary, the record hibernating with no `currentExecutionId`, and the next process resuming it into an idle session that takes another send WITHOUT `onInterruptedExecution` firing.
- `src/__tests__/session-residency.spec.tsx` — the states between live and gone: an eviction stamping `hibernated` where a genuine close stamps `closed`, the store's prune sweep passing over a hibernated record and taking the closed one, `resumeSession` rebuilding an evicted session idle and able to run another turn with its identity (principal, metadata) read back off the record while the construction-bound scope ceiling does not survive, a resume from a record written by a previous process adopting rather than blanking it, two concurrent resumes collapsing onto ONE construction, `undefined` for an id never opened / already ended / destroyed, `closeSession` dropping the registry entry — including after a `session.close()` behind the app's back — so reopening the id yields a live session rather than the corpse, and `closeApp` hibernating a live session (which a fresh app over the same stores resumes, knobs included, and runs a turn on) while a session closed before the shutdown stays `closed` and unresumable.
