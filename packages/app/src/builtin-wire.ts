 ...
 * bundled built-ins and the gateway's wire: `@agentick/app` depends on the built-in
 * harness packages, so it can name their wire-extensions; the gateway depends
 * on `@agentick/app`, so it can register them — without the gateway ever importing a
