...
Aliases exist because a uri is not a name: it is documented, in server instructions and in prose the model reads, so rewriting it makes the model's most reliable source of truth wrong. An alias resolves but is **not** a catalog entry — `list()` reports one row per registration and `has()` answers only for registered uris. Two registrations claiming one alias is an error, not a race: the read fails naming both candidates rather than handing back whichever registered first, and un-registering one claimant makes the alias unambiguous again.
...
- **An ambiguous alias is not branchable.** The read fails and the message names both claimants, but `ResourceAliasAmbiguous` is currently re-wrapped as `ResourceResolverFailed` on the way out of `read`, so code cannot switch on the tag.
