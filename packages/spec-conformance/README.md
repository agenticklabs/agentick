...
| `runExecutorConformance(factory, errorFixtures)` | `LanguageModelExecutor`         | Project/run phase isolation, abort, cross-adapter parity (base64 image sources, sampling params, `providerOptions` threading, the stream surface), and provider-error classification end-to-end |
...
- **`errorFixtures` drives one call.** The classification block asserts the tag on `execute()`'s rejection. An adapter that classifies differently on the streaming path would pass here and fail in production.
