...
- **`runRecoveryConformance` runs for all four shipped adapters.** [@agentick/model-google](../model-google), [@agentick/model-ai-sdk](../model-ai-sdk), and [@agentick/model-openai](../model-openai) cover both seams. [@agentick/model-anthropic](../model-anthropic) covers streaming and passes `{ nonStreaming: false }`: `messages.create` returns `tool_use.input` server-parsed, so an unparseable argument buffer cannot occur on that seam — the one skip in the family is a fact about the provider, not a gap in an adapter.
