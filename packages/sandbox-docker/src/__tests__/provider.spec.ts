...
describe.skipIf(!hasDocker)("dockerProvider", () => {
  ...
  it("creates a sandbox with valid id and workspacePath", async () => {
  ...
  it("handles files with special characters", async () => {
    ...
    await sandbox.writeFile("special.txt", specialContent);
    const content = await sandbox.readFile("special.txt");
  ...
  it("throws ENOENT for missing files", async () => {
    ...
    await expect(sandbox.readFile("nonexistent.txt")).rejects.toThrow("ENOENT");
  });

  it("provider name is docker", () => {
    const provider = dockerProvider();
    expect(provider.name).toBe("docker");
  });

  it("uses custom workspace path", async () => {
    const provider = dockerProvider({ workspacePath: "/app" });
    const sandbox = await provider.create({ workspace: true });
    sandboxes.push(sandbox);

    expect(sandbox.workspacePath).toBe("/app");

    await sandbox.writeFile("test.txt", "custom path");
    const result = await sandbox.exec("cat /app/test.txt");
