...
wire impl built on
[`@agentick/cluster-broker`](../cluster-broker) + Node's `net`
module.
...
```typescript
import { defineTcpCluster, defineUnixCluster } from "@agentick/cluster-net";
...
```typescript
import { tcpBroker } from "@agentick/cluster-net";
...
import { BaseBroker } from "@agentick/cluster-broker";
...
multi-host HA reach for `@agentick/cluster-redis` (Redis is the
