...
Skills are first-class data. The harness treats them as opaque content; the agent's prompt design decides what to do with them. Authoring `content` via `renderTemplate` from `@agentick/compiler-react` (JSX → markdown string) is the recommended pattern for richer skill bodies.
...
// The SkillStore / SkillStoreQuery ports live in @agentick/spec.
...
The reference `SkillsHarness` keeps skills in memory (with snapshot/restore). The protocol is backend-agnostic — `SkillsHarnessProtocol` in `@agentick/spec` defines the shape; alternative implementations slot in:
...
- [`@agentick/compiler-react` `renderTemplate`](../compiler-react/README.md) — author skill `content` as JSX templates
