...
export function createRoot(container: TentickleContainer): FiberRoot {
  // react-reconciler 0.29.x expects 10 arguments
  return reconciler.createContainer(
    ...
    // onCaughtError
    (error: Error) => {
      // Error caught by error boundary - typically silent
    },
    // onRecoverableError
    (error: Error) => {
      console.warn("Recoverable error in Tentickle:", error);
    },
    // onDefaultTransitionIndicator (or transitionCallbacks depending on version)
    null as any,
  );
}
