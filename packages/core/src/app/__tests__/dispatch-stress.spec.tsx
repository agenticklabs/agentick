/**
 * dispatch() Stress & Edge Case Tests
 *
 ...
 * - audience: "user" tool truly invisible after multiple ticks
 */
...
describe("concurrent dispatch", () => {
  it("should handle multiple concurrent dispatch calls", async () => {
    let callCount = 0;
    ...
    const results = await Promise.all([
      session.dispatch("counter", { id: "a" }),
      session.dispatch("counter", { id: "b" }),
      session.dispatch("counter", { id: "c" }),
      session.dispatch("counter", { id: "d" }),
      session.dispatch("counter", { id: "e" }),
    ]);
  ...
  it("should not double-mount when concurrent dispatches trigger mount", async () => {
    ...
    const [r1, r2] = await Promise.all([session.dispatch("spy", {}), session.dispatch("spy", {})]);
...
describe("error propagation", () => {
  it("should propagate handler errors as rejections", async () => {
    ...
    await expect(session.dispatch("exploder", {})).rejects.toThrow("kaboom");
  ...
  it("should validate input against tool schema", async () => {
    ...
    await expect(session.dispatch("strict", {})).rejects.toThrow("Validation failed");

    // Valid input works correctly
    const good = await session.dispatch("strict", { count: 5, name: "test" });
...
describe("lifecycle coherence", () => {
  it("audience: 'user' tool should not appear in model input across multiple ticks", async () => {
    ...
    const result = await session.dispatch("hidden", {});
  ...
  it("dispatch should work between ticks of a multi-tick execution", async () => {
    ...
    const r1 = await session.dispatch("state-tool", {});
    ...
    const r2 = await session.dispatch("state-tool", {});
  ...
  it("mount-only session should not affect subsequent render()", async () => {
    ...
    const dr = await session.dispatch("mt", {});
...
describe("stateful user-audience tools", () => {
  it("should support tools that use React state via use()", async () => {
    ...
    const result = await session.dispatch("stateful", { value: "hello" });
...
describe("session lifecycle edge cases", () => {
  it("should throw when dispatching to a closed session", async () => {
    ...
    // Works before close
    const r = await session.dispatch("simple", {});
    expect(r).toEqual([{ type: "text", text: "ok" }]);

    await session.close();

    // After close — must throw
    await expect(session.dispatch("simple", {})).rejects.toThrow("Session is closed");
  });
  ...
  it("should handle dispatch of tool that exists as alias AND as another tool name", async () => {
    ...
    // Name lookup takes priority over alias lookup in dispatch.
    const result = await session.dispatch("shared", {});
    expect(result).toEqual([{ type: "text", text: "from-b" }]);

    // tool-a is still accessible by its primary name
    const resultA = await session.dispatch("tool-a", {});
