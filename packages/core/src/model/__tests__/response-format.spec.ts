...
describe("ResponseFormat", () => {
  ...
  describe("normalizeModelInput", () => {
    ...
    it("is undefined when not provided", async () => {
      const normalized = await normalizeModelInput(baseInput, baseConfig);
      expect(normalized.responseFormat).toBeUndefined();
    });

    it("input responseFormat overrides config responseFormat", async () => {
      ...
      const normalized = await normalizeModelInput(input, config);
      // Spread order: {...defaults, ...input} → input wins
      expect(normalized.responseFormat).toEqual({ type: "json" });
    });

    it("uses config responseFormat as fallback", async () => {
      const config: ModelConfig = {
        model: "test-model",
        responseFormat: { type: "json" },
      };
      // Input without responseFormat
      const _normalized = await normalizeModelInput(baseInput, config);
