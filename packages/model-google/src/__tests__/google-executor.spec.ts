...
describe("google() adapter — non-streaming", () => {
  ...
  // The tool-call rejection family is NOT a stop reason — it is a typed
  // failure a retry policy can read (ADR 99). See `malformed-tool-call.spec.ts`.
  it("fails the run with MalformedModelOutput on finishReason=MISSING_THOUGHT_SIGNATURE", async () => {
    const stub = new StubGoogleClient([
      {
        kind: "non-streaming",
