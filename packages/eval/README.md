...
`@agentick/app` and pass their own factory).
...
**`@agentick/eval/plugins/judge`** — LLM-as-judge. Model-agnostic:
...
Write your own by mirroring either: `declare module "@agentick/eval"
...
```ts
import { renderHtmlReport } from "@agentick/eval";
...
```ts
import { structuredOutputComplianceEval } from "@agentick/eval/__tests__/structured-output-compliance.example";
