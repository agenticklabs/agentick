...
- `src/__tests__/store-backing.spec.ts` — every write path reaching the store, scope partitioning across two harnesses on one store, `hydrate()` repopulating and pinging subscribers, `persist()` awaiting in-flight writes, the values lost when the default per-harness store is used, the construction seed writing through and upserting over a hydrated knob, and `hydrate`/`persist` driving a seam-only store.
