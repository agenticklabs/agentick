...
**Prompt caching, two granularities.** A per-block breakpoint comes from `providerMetadata.anthropic.cacheControl` on the specific content block; a per-tool one from `ProviderToolOptions["anthropic"].cache_control`. An explicit per-block `cacheControl` beats the canonical `CacheBoundary` the executor otherwise translates onto the last block — a boundary's `ttlMs` becomes `ttl: "5m"` at five minutes or less and `"1h"` above. The target declares what it takes — `cache: { ttlMs: 5 min, extendedTtlMs: 1 h, refreshedOnRead: true, explicit: { kind: "breakpoint", maxBoundaries: 4 } }` — so the executor's screen, not this adapter, enforces the four-breakpoint limit and the longest-lifetime-first order.
...
- `src/__tests__/cache-declaration.spec.ts` — the target's cache record states what the provider publishes: a five-minute prefix, one hour extended, refreshed on read, four explicit breakpoints.
