...
- **`useKnob` reactive state** — the model can flip `verbose` via `knob_set`; the next render's system prompt changes.
...
2. **Turn 2** — asks "is it done yet? if not, await it." The model calls `task_get` to check status, then `task_await` to block until the deploy task transitions to `completed` — and reports the final result.
3. **Turn 3** — asks the model to lint the repo via the MCP-discovered `demo__lint_repo` tool and wait for the result. The MCP server returns a `CreateTaskResult` over the wire on `tools/call`, emits `notifications/tasks/status` updates while it works, and finally serves the result via `tasks/result`. The `withMCP` client mirrors the remote task into the local `TasksHarness`; the model drives it with the same `task_*` tools it used in turn 2 — there's no client-side difference between a local Pattern B task and an MCP-remote Pattern B task.
