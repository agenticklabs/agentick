...
`@agentick/client` (the batteries-included bundle), and every built-in
...
because `@agentick/client` side-effect-imports the harness `/client`
subpaths — install-to-appear (ADR 87). Use `@agentick/client-core`
...
The same agent is scored by `@agentick/eval` — and because its output is
...
`t.sh` / `t.file` come from `@agentick/eval/plugins/workspace`; `t.judge`
