...
- **`packages/reconciler-react/src/contributors/`** — the existing contributors (semantic HTML, content blocks, formatters)
- **`packages/reconciler-react/ARCHITECTURE.md`** if present, or the reconciler-react README
...
import type { Contributor } from "@agentick/reconciler";
...
The exact shape depends on the contributor type. Read the existing contributors in `packages/reconciler-react/src/contributors/` for the canonical shapes.
...
5. **Trying to depend on `@agentick/reconciler-react` from a non-React surface.** Per ADR 27, your harness package depends on `@agentick/reconciler-react` only via the `/react` subpath. The base package must work without React.
