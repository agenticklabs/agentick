# @agentick/client-offline-next

Offline queue middleware for `@agentick/client-next`. Buffers
outbound RPCs when the wire is closed; replays FIFO on reconnect.
...
| Outbox pattern (Hohpe & Woolf, *Enterprise Integration Patterns*) | Reliable async messaging across failure domains | Conceptual basis |
...
queue. Pair with `@agentick/client-retry-next` to get idempotency-key
emission on non-idempotent methods:
...
```ts
import type { OfflineStore, QueuedRequest } from "@agentick/client-offline-next";
...
- **IndexedDB store** — `@agentick/client-offline-indexeddb-next`. Not
  shipped; defer until a real browser adopter surfaces.
- **SQLite store** — `@agentick/client-offline-sqlite-next`. Not
  shipped.
