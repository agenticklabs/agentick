...
import type { MetricSink } from "@agentick/runtime-next";
import type {
  MetricLabels,
  TelemetryLayer,
  TelemetryOptions,
 ...
 * ```
 *
 * @verifiedBy packages-next/app/src/__tests__/telemetry-wiring.spec.ts
 */
 ...
 * `@agentick/telemetry-otlp-next` package.
 *
 ...
 * variable so the compiler does not require `@agentick/telemetry-otlp-next` as
 * a build dependency of `@agentick/app-next` (it is an OPTIONAL install).
 */
async function tryAutodiscoverOtlp(): Promise<TelemetrySink | undefined> {
  const specifier = "@agentick/telemetry-otlp-next";
  try {
    ...
    console.warn(
      "[agentick] telemetry: OTEL_EXPORTER_OTLP_ENDPOINT is set but " +
        "`@agentick/telemetry-otlp-next` is not installed — no OTLP export. " +
