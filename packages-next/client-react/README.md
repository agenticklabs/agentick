...
> `@agentick/compiler-react-next` (`BridgeProvider` / `useBridges`) is the
...
```bash
npm install @agentick/client-react-next @agentick/client-next react
```
...
```tsx
import { useHandle } from "@agentick/client-react-next";
...
```tsx
import { useView } from "@agentick/client-react-next";
...
> A related fix landed in `@agentick/client-core-next` as part of this slice:
...
| `filteredView.list()` is referentially stable between changes | `@agentick/client-core-next` `src/__tests__/view-source.spec.ts` |
