...
describe("@agentick/spec-next — compiler protocol", () => {
  ...
  describe("CompilerInboxMessage", () => {
    it("supports recompile / unmount / invalidate", () => {
      const ms: CompilerInboxMessage[] = [
  ...
  describe("CompilerProtocol shape", () => {
    ...
    it("lifecycle projection is an OPTIONAL capability, not a protocol method (ADR 89 §4)", () => {
      // `notifyLifecycle` is retired: `keyof CompilerProtocol` must not
      // admit it (compile-time), and a compiler that projects lifecycle
      // implements `LifecycleProjectionTarget` separately.
      type NotAMethod = "notifyLifecycle" extends keyof CompilerProtocol ? true : false;
