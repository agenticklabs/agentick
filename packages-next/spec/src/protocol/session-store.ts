 ...
 * and the `runSessionStoreConformance` suite live in `@agentick/session-next`,
 ...
 * `@agentick/session-next`), exactly like the task / timeline stores.
 *
 * Bundled default: `InMemorySessionStore` (`@agentick/session-next`). A
 * `@agentick/session-store-postgres-next` conforms to this SAME port later —
