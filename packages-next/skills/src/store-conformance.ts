 ...
 * `@agentick/skills-store-postgres-next`, a filesystem source, any adopter store
 ...
 *
 * ```ts
 * import { runSkillStoreConformance } from "@agentick/skills-next";
...
import type { Skill, SkillStore } from "@agentick/spec-next";
  ...
  runStoreConformance<SkillStore>({
    ...
    cases: ({ setup }) => {
      it("put then get round-trips the record (name-keyed)", async () => {
        ...
        await store.put(r);
        expect(await store.get("git_push")).toEqual(r);
      });

      it("put upserts in place — a later put of the same name replaces", async () => {
        const store = await setup();
        await store.put(skill("git_push", { description: "old" }));
        await store.put(skill("git_push", { description: "new", content: "new body" }));
        const got = await store.get("git_push");
      ...
      it("list() with no query returns every record", async () => {
        const store = await setup();
        await store.put(skill("a"));
        await store.put(skill("b"));
        expect((await store.list()).map((s) => s.name).sort()).toEqual(["a", "b"]);
      });

      it("list(query) filters by substring across name + description (case-insensitive)", async () => {
        const store = await setup();
        await store.put(skill("git_push", { description: "Push to remote" }));
        await store.put(skill("git_pull", { description: "Pull from remote" }));
        await store.put(skill("docker_build", { description: "Build an image" }));
        const got = await store.list({ query: "REMOTE" });
        expect(got.map((s) => s.name).sort()).toEqual(["git_pull", "git_push"]);
      });

      it("list(query) filters by tagsAny (OR) and tagsAll (AND)", async () => {
        const store = await setup();
        await store.put(skill("git_push", { tags: ["git", "vcs"] }));
        await store.put(skill("ssh_to_host", { tags: ["ssh", "network"] }));
        await store.put(skill("docker_build", { tags: ["docker"] }));
        const any = await store.list({ tagsAny: ["vcs", "docker"] });
        expect(any.map((s) => s.name).sort()).toEqual(["docker_build", "git_push"]);
        const all = await store.list({ tagsAll: ["git", "vcs"] });
        expect(all.map((s) => s.name)).toEqual(["git_push"]);
        const none = await store.list({ tagsAll: ["git", "docker"] });
        expect(none).toEqual([]);
      });

      it("delete() removes a record and is idempotent", async () => {
        const store = await setup();
        await store.put(skill("fade"));
