/**
 * Wire `@agentick/spec-conformance-next` against `@agentick/compiler-react-next`.
 *
 ...
 * package — `runKnobsHarnessConformance` lives in `@agentick/knobs-next`
 * and runs against the real `KnobsHarness` from
 * `@agentick/knobs-next/src/__tests__/harness.spec.ts` (and likewise for
 * state and timeline).
 *
...
import type { HookBridges, CompilerProtocol } from "@agentick/spec-next";
import { InMemoryDataBridge } from "@agentick/compiler-next";
import { fakeBridges, stubLoopBridge } from "@agentick/compiler-next";
