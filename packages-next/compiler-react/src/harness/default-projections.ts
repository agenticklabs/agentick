 ...
 * `@agentick/compiler-next` (`builtInToolsProjection`). The `timeline`
 * default needs a live timeline harness, so it is contributed here by the
 * compiler binding.
 *
 * Per ADR 27, `@agentick/compiler-react-next` has NO dependency on
 ...
 * (no `@agentick/resources-next` / `@agentick/mcp-next` import — ADR 27):
...
import type { ContentBlock, HookBridges, MessageEntry, SectionEntry } from "@agentick/spec-next";
import type { DefaultProjection } from "@agentick/compiler-next";
 ...
 * present. NO `@agentick/mcp-next` import — pure duck-typing (ADR 27).
 */
