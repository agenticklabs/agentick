 ...
 *
 * Connects the compiler to the standalone React DevTools application
 * (`npx react-devtools`, default port 8097). Each created compiler
 * registers itself with the global DevTools hook via
 * `injectIntoDevTools` — no opt-in per compiler is required, just
 ...
 * @example
 * ```ts
 * import { enableReactDevTools, CompilerHarness } from "@agentick/compiler-react-next";
 *
