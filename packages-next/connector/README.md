...
A platform that wants those composes `@agentick/formatters-next`
(`summarizedFormatter` / `textOnlyFormatter`) + `splitMessage` from
`@agentick/utils-next` itself.
...
- **Platform ports** — `@agentick/connector-telegram-next` /
  `@agentick/connector-imessage-next` are deferred (gated on their
