 ...
 *
 * @verifiedBy packages-next/resources/src/client/__tests__/resources-handle.spec.ts
 * @verifiedBy packages-next/resources/src/client/__tests__/session-resources.spec.ts
 */
...
export function resourcesHandle(
  ...
  // happened before I connected". Fire-and-forget; errors are swallowed (a poll
  // may fail before the session is reachable — an explicit `refresh()` recovers).
