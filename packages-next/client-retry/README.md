# @agentick/client-retry-next

Retry middleware for `@agentick/client-next`. Exponential backoff with
...
- **Circuit breaker** — stop retrying entire methods when failure rate
  ...
  ship as `@agentick/client-circuit-breaker-next` if demand surfaces.
