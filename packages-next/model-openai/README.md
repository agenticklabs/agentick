...
Model selection: `target.modelId` (the per-tick `<Model>` override, ADR 56) wins over the construction-time `openai(model)` default; the default
...
**Provenance-half: citations DONE, tool-result executedBy NARROWED.** On the
Chat Completions surface this adapter targets, web-search provenance arrives as
`message.annotations` (`url_citation`). These are mapped onto the canonical
`Citation[]` on the assistant text block (`url`/`title` → `source`,
`start_index`/`end_index` → `range`).

Stamping a `tool_result` with `executedBy: "provider:openai"` is **narrowed**
(`TODO(pass-d)` in `normalizeImpl`): Chat Completions does NOT surface
provider-executed tool RESULTS as discrete items — those are typed output items
on the **Responses API**, which this adapter does not target. `message.tool_calls`
only carries dispatchable `type: "function"` calls, so there is no handler-less
provider call to suppress on this surface either. Per the HARD RULE, we stamp
what the API returns (citations) and narrow-TODO what the surface cannot reach.
...
- `src/__tests__/openai-executor.spec.ts` — dialect behavior (message
  ...
  provider-tools request-half, Pass D provenance-half web-search
  citations).
