 ...
 *
 * The slot is spec-resident plain data (no `@agentick/model-next` dep), so
 * this hook keeps `reconciler-react` free of the model layer — mirroring
 * the deliberate choice made for `contextInfo`.
 *
 * @see packages-next/reconciler-react/src/react/hooks/use-context-info.ts
 */
