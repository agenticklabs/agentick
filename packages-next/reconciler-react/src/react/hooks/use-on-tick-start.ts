 ...
 * the harness's `LifecycleStore` catches them up by invoking the
 ...
 * iteration.
 *
 * @see packages/reconciler-react/src/harness/lifecycle-store.ts
 */
...
export function useOnTickStart(
  ...
  useEffect(() => {
    return store.register("tick-start", (event) => ref.current(event));
  }, [store]);
}
