 ...
 *
 * Mirrors what `@agentick/transport-next`'s `buildWireExtensionContext` hands the
 * dispatcher, minus the transport-owned `transport` plumbing (stubbed here) — a
 * gateway unit test needs no live connection.
 */

import type { GatewayHarnessProtocol, WireExtensionContext } from "@agentick/spec-next";
