...
so that [`@agentick/app-next`](../app) stays exporter-dep-free. The app
...
```ts
import { createApp, createTelemetry } from "@agentick/app-next";
...
and NO exporter is wired, `@agentick/app-next` lazy-imports this package and
...
- **The exporter deps live HERE, not in the app.** `@agentick/app-next` declares
...
- The `TelemetrySink` contract this package implements lives in
  [`packages-next/spec/src/protocol/app-harness.ts`](../spec/src/protocol/app-harness.ts).
