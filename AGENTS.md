...
- **`@agentick/compiler-react` has NO harness deps.** It owns the JSX → IR pipeline and the bridge context (`BridgeProvider` / `useBridges`); the reference `InMemoryDataBridge` lives in `@agentick/compiler`. The checkpoint fold iterates `HookBridges` generically via `CheckpointCapable` feature detection — no hardcoded slot names. Any harness adds a `/react` subpath depending on compiler-react without a cycle.
