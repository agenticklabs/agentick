...
**Status of the audit itself:** all six dimensions complete. The content-block×adapter matrix (which first failed its structured-output cap) was re-run as a focused agent and its findings are **verified in code by Fable** (§Content-block matrix below) — it surfaced the audit's single most important finding.
...
**New cut-blockers (net-new, not in the original 15):**

- **CB-BLOCKER-1 · Anthropic extended-thinking + tool-use round-trip broken (both directions).** Output: `anthropic-adapter.ts:1057-1069` maps `thinking→{reasoning,text}` but never captures `block.signature`, and `redacted_thinking` does `void (block)` discarding opaque `data` (verified). Input: no `reasoning` variant on `LanguageModelMessagePart` to replay it. Anthropic _requires_ the signed thinking block be replayed verbatim next turn when tools are active → API errors / silent CoT loss on the flagship provider. **Cut-blocking correctness.**
- **CB-BLOCKER-2 · `document` dropped by all four adapters — confirmed v1 regression.** v1 had native PDF/document support (Anthropic `packages/adapters/anthropic/src/anthropic.ts:537-563`, OpenAI `openai.ts:565-584`, Google `google.ts:454-473`); v2 has no `document` case anywhere. The exact bug class this audit was chartered to catch.

**Also (major, non-blocking):** `generated_image`/`generated_file` → `JSON.stringify` dumps base64 into a text part (token bomb); ai-sdk drops reasoning output (v1 regression); Google loses its own response `inlineData`/`executableCode`/`codeExecutionResult`; `audio`/`video` input dropped where OpenAI/Gemini support them.

**Fix shape (one change re-enables native projection everywhere):** add `document`, `audio`, `video`, and `reasoning` (carrying `signature`/`data`) variants to `LanguageModelMessagePart` (`executor.ts:227`); then the two Stage-1 mappers (`canonical-projection.ts:136`, `anthropic-adapter.ts:918`) and the four adapter switches get the corresponding cases. Closes CB-BLOCKER-2 + MAJOR media/generated drops together. CB-BLOCKER-1's output side is a separate per-adapter `normalize` fix. `json`/`xml`/`csv`/`html`/`code`/event blocks flatten-to-text **by design** (the text formatter owns them) — not bugs.

**Matrix (INPUT projection fate):** text/image/tool_use/tool_result ✅ all four · document ❌ all four (v1 regression) · audio/video ❌ all (🚫 Anthropic→JSON) · reasoning ⚠️ collapses, loses signature · generated_image/generated_file/executable_code/code_execution_result/custom ❌ all · json/xml/csv/html/code/events ⚠️ by-design flatten. **OUTPUT:** Anthropic loses thinking `signature` + redacted `data`; ai-sdk maps neither reasoning nor files; Google drops response inlineData/executableCode.

## Executive summary

**Updated:** the content-block matrix (above) adds 2 net-new cut-blockers (CB-BLOCKER-1/2) and is the single highest-priority correctness finding — total cut-blockers now **17**. The synthesis below covers the other four dimensions.
