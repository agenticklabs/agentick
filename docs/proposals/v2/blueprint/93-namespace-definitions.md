...
a cron job) already exists and stays. These namespaces earn membership
on governance + legibility: declared ingress joins the op grammar
(guard-vetoable, journaled fires — a hand-rolled webhook send is
un-guarded, un-journaled ingress) and reads at a glance in the grammar
tree (`subscriptions.ts` = what wakes this agent).
