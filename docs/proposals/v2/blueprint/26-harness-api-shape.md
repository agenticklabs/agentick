...
**Touches:** Every harness protocol interface in `@agentick/spec-next` (`SessionHarnessProtocol`, `AppHarnessProtocol`, `ReconcilerProtocol`, `ToolExecutorProtocol`, `LoopExecutorProtocol`, sandbox + MCP harness protocols), `HookBridges` (which becomes a bag of harnesses), the concrete implementations across the workspace (`@agentick/app-next`, `@agentick/session-next`, `@agentick/reconciler-react-next`, `@agentick/tool-executor-next`, `@agentick/model-executor-next`, `@agentick/loop-executor-next`, `@agentick/sandbox/v2`), every conformance suite, every example, every test.
