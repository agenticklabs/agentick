...
- **Tree-positional context** — a value set by an *ancestor provider* at a specific tree
...
- **the executor stays harness-agnostic** — it takes one generic construction option,
  ...
  builds. It never imports sandbox. The *type* of `ctx.sandbox` comes from the
  augmentation; the *value* comes from the wiring layer.
...
the dispatch *input*, a dispatch's caller-supplied input is cleanly `{ name, input, scope }`
— serializable. That unblocks:

- **dispatch-as-command** — `tool:command:dispatch` becomes a real `this.command()`:
  journaled, provenance/principal-stamped (dispatch is *the* model-originated action, the
