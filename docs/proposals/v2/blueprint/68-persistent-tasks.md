...
timeline stores. **Ship the in-memory default now; a `@agentick/tasks-postgres-next` conforms
...
interface TaskExecutor {
  ...
  start(record: TaskRecord, work: TaskWork, report: (u: TaskTransition) => void,
        signal: AbortSignal): TaskExecution;
...
- **Delivers now:** the record/executor architecture (the on-ramp); an app-scoped durable-
  *within-a-run* ledger; `detached` tasks that outlive their session; two execution strategies
...
where submitted); cross-node *execution* is the ambitious distributed-worker tier. The
...
Deferred, seam-ready: `@agentick/tasks-postgres-next`; sandbox + distributed-worker executors;
