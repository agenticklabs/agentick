...
capability of the substrate inbox** — a thin forwarding discipline on the inbox's _existing_
...
transform — "interception") or **forwards** it to _its own_ escalation-parent; the **root**
...
- **Elicitation can't leave its session or its tick.** `ElicitationHarness` is per-session
  ...
  `elicitation/create` issued _during a live call_. A task that has outlived its tick, or a
  sub-agent whose "client" is really its _spawner_, has nothing to elicit through.
...
- **The pattern is payload-agnostic and recurs.** Sampling, permission, resource, and error
  escalation are the _same_ relay with a different payload + terminal. Embedding the relay in the
...
`ask`** — the `ask` return-value stack _is_ the relay AND the return path; there is no separate
...
no ancestor intercepting, the chain **behaves like direct-to-client**, preserving only the _seam_.
...
fiber registry). A **forked child** task has a _separate_ inbox — so T1 (in-process) is trivial
...
for a spawned session, **absent (terminal)** at a root session — _distinct from_ `parent`.
...
"Tasks that outlive their session" is thus a _narrow_ mode: fire-and-forget, **non-interactive**,
...
human sees anything. **Default: bubble.** Cluster addressability _enables_ a **direct-delivery
...
ADR 51 — the human sees _who_ is asking through _what_ authorized chain. Any hop may **deny or
...
- **Free future riders (enabled, not built): sampling** (escalate a completion to an ancestor that
  ...
  credentials never cross the wire — the _request_ escalates; the credential stays server-side),
  **error escalation**. Each is a payload class + a terminal resolver; the relay is shared.

## Tiers (build order)

- **T1 — root-session task → client.** A task in a _root_ session (connected client) escalates to
...
- **MCP wire verb for a client supplying input to an `input_required` task.** MCP's task+elicitation
  ...
  the spec allows, rather than implement a settled one. (Terminal resolution over the _local_ client
