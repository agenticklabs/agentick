...
at construction. That is one concept — *intercepting an operation* — wearing
three costumes.
...
```ts
class OperationVeto    { _signal = "veto"    ; reason? }
class OperationDefer   { _signal = "defer"   ; retryAfter? }
class OperationReplace { _signal = "replace" ; result; reason? }
```
...
succeeds with the result). Only the *trigger* changed — a caught signal instead
of a separate phase.
...
also *fixes* a latent gap: per-session sub-harnesses now inherit app-level
...
*same* scope with a `replace` composed before a `veto`; that was untested and is
...
- **A shared `Cascade<T>` container for guards + transforms + hooks.** Guards and
  ...
  unification is the *strategy* (fold-at-construction, read-local-per-op), not a
  container type.
...
- `LifecycleHandlerError` (spec) is retained as a valid error-taxonomy entry but
  currently has no producer: a guard decider that *throws* (rather than returning
