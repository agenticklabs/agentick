...
**A finding worth Ryan's attention: the bare specifier does NOT resolve under pnpm's strict linking.** `packages/code/node_modules` holds only code's declared deps, so `resolveDefaultRuntime()` returned `undefined` in-workspace on the first measurement — the zero-config default would have been silently inert for every test, every example, and every pnpm adopter who got `code-host` transitively rather than directly. Fixed for the workspace by adding `@agentick/code-host` to the ROOT `devDependencies`, which node's upward walk finds (`packages/code/src` → … → `<root>/node_modules`) and which adds no published edge and no turbo cycle (verified: 6 tasks, no cycle error). That makes the zero-config pins REAL — `code: {}` spawns an actual subprocess and returns 42 through the app. **It does not fix the adopter case**, which is documented in both READMEs (install it as your own dependency, or pass `runtime` explicitly and never guess) and is the thing to revisit when the metapackage lands: an optional peerDependency or a side-effect registration from `code-host` would both close it properly.
