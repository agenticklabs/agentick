...
**2026-08-23 — interruption detection is a property of REHYDRATING, not of the resume door (uncommitted, `feat/v2`).**

`app.resumeSession(id)` ran the two-signal crash detection; `app.createSession({ sessionId })` on an id with a durable record rehydrated it and ran nothing — so a crashed turn reopened through the create door (in-process adopters, and any host using create-as-open) was silently forgotten. That is a second recovery path wearing the first one's clothes, which `checkpointing.md §4` exists to forbid. Both doors now call ONE `AppHarness.reconcileInterruption(sessionId)`; the resume door's sequence was factored, not copied, so the two cannot drift.

**The load-bearing move is WHERE signal 1 comes from.** It used to be captured pre-construction by `rebuildFromRecord` — fine for resume, which reads the record anyway to answer "does this session exist", but on the create door it would have meant a store probe on every explicitly-id'd create, including the fresh ones that can never have a record. `SessionRuntime.hydrate()` already reads the record; it now KEEPS it, and `takeAdoptedRecord()` (through `SessionHarness`, the `flushRecordWrites` precedent) hands it up once. Detection therefore costs a fresh create nothing — pinned by a counting store asserting exactly one record read — and `rebuildFromRecord` shrinks back to what its name says. The one-shot claim is what makes the create door's idempotent open-or-rehydrate safe: a second `createSession` on the same live id returns the live session and the policy does not fire twice.

Everything else is preserved by construction, because it is the same code: the F1 flush barrier before the mark-put (the write-behind clobber that InMemory masks), `resumeAttempts` bump-or-reset keyed to the execution id, `running`-only candidacy, boundary-present ⇒ no mark and no callback, and a throwing policy rejecting the open rather than degrading to `drop`. Destroy's rebuild still reconciles nothing — it never claims the record.

Five pins in `interrupted-callback.spec.tsx` (create-door crash producing the SAME observed sequence as the same crash through resume, and firing once across a repeat open; boundary via create; fresh create's single read; `resumeAttempts` 1→2 across a re-crash; a throwing policy rejecting the create). Discrimination checked BOTH ways: without the create-door call the three positive pins fail; with signal 2 ignored the two boundary pins fail; with a pre-construction store probe the read-count pin fails. One test-shape note worth keeping: `expect(app.createSession(...)).rejects` on a resolved open makes vitest diff a whole live session harness and OOM the worker — assert on a collapsed string instead.

Net −10 lines in `packages/app/src/harness.ts` (+27 in `@agentick/session` for the adopted-record surface, and a `TODO(durable-spawns)` on the spawn door, which deliberately does not reconcile — §8's re-attach-by-stable-id is the child's path). Gates: `npx vitest run packages/app packages/session` 92 files / 603 tests green; root `npx vitest run` 744/748 files green with one unrelated flake (`model-executor` `base-effect-stream` abort timing, green in isolation); `pnpm typecheck` 111/111; oxfmt + oxlint clean; NUL byte-scan clean on all four changed files.

**2026-08-23 — session-doors residuals: a typed gateway fake, channel frames stamped, and the `door` sweep that found nothing to rename (uncommitted, `feat/v2`).**

Three recorded residuals from the session-doors / ADR 102 arc, cleared together.

**`fakeGatewayHarness()` replaces every `as unknown as GatewayHarnessProtocol`.** Stage 2/3 added three protocol members at once (`sessionNodeFor`, `attachableNodesFor`, `attachScopeNode`) and nine cast-through doubles across seven packages silently did not gain them — a double that lies fails at runtime, in whichever test happens to reach the member, not at the change that caused it. All nine now build through a fake typed against spec, so the next member addition breaks compilation once. `apps` takes the array and powers `apps()` / `app(id)` / `appForSession(id)` / `destroySession(id)` together, which is the shape every call site was hand-rolling; every protocol member is overridable by its real signature. `attachScopeNode` throws without a `bus` option rather than leasing over a bus that does not exist — loud beats a no-op lease.

**It lives in `@agentick/spec-conformance`, not `@agentick/gateway`.** The brief proposed gateway's `/testing`; the dep graph forbids it. `@agentick/gateway` dev-depends on `spec-conformance`, so a fake there could not reach `spec-conformance`'s own placeholder host without a cycle, and the harness packages that need it (tasks, knobs, live, completions) are leafward of the gateway — a manifest edge from them to the gateway is the generic→specific direction we reject. The protocol is spec's; the fake belongs with spec's other fixture, `fakeToolHandlerCtx`, which is the same thing for `ToolHandlerCtx` and set the precedent. Zero cast sites were left behind.

**Channel frames now stamp the principal, via one stamper instead of a third copy.** `BaseHarness.request()` built its envelope from `opts.scope` with no principal merge — the last unstamped emitter from #304, and `notifyChannel()`, its fire-and-forget twin, had the identical gap. Rather than paste the merge a third and fourth time, the shape `emitSignal` was already apologizing for duplicating is extracted as `stampScope()` and all three emitters route through it: net fewer lines, one place where "gap-fill `parentScope`, assert `principal`" is written. Pinned by two cases in `base-harness-principal.spec.ts` (both frames stamped; a principal-less harness still relays untouched). Discrimination checked: with `request()` reverted to `opts.scope ?? {}`, the stamping case fails and the other six pass.

**The elicitation half of that residual is NOT closed, and it is one line elsewhere.** `AppHarness.createSession` constructs `ElicitationHarness` with `{ parentScope: { sessionId } }` and no `principal`, so `this.principal` is `undefined` there and the new stamp is inert for elicitation specifically until that construction threads it — `input.principal` is already in hand at that call site. Left alone because `packages/app/src/harness.ts` was under concurrent edit. The mechanism is in place and correct for every harness that carries a principal; only the construction is missing.

**The `door` sweep found no identifiers to rename.** `dde43821e` added `via` and `envelope` — already concepts, not metaphors. Every `door` hit in `@agentick/tool-executor` and `@agentick/spec` is prose (docblocks, README headings, test names), which the naming ruling permits. The one genuine violation in the workspace was elsewhere and is renamed: `PromptsCompleteDoor` / `promptsDoorOf` in `@agentick/completions`'s wire route → `PromptsCompleteCapable` / `promptsCompleteCapableOf`, matching `CheckpointCapable`, which that very docblock cites as the pattern it follows. Private to the file, five references, no API change.

Gates: root `npx vitest run` 745 files / 7389 tests green, 0 failed (both known-flaky suites passed in the same run); `pnpm typecheck` 111/111; oxfmt on every changed file; NUL byte-scan clean over all 20 changed source/doc files.

**2026-08-23 — ADR 102 stage 3: `session` scope is an attachment, so a subscription resolves no session (uncommitted, `feat/v2`).**

The `session` arm of `openScopeEvents` was `ownerApp(...)` → 404-on-miss → a `sessionId` query over the owning app's ring. It is now the same own-node attachment `gateway`/`app` take, with the caller's query narrowed by `scope: { sessionId }`. That single change makes LIVE, EVICTED and NEVER-CREATED ids one shape — no registry lookup, no app resolution, no `SessionNotFound` — which is `session-doors.md` slice 2 delivered as a consequence of topology rather than as admission machinery. The doors doc's §4 (an attachment authorizer plus `appId`-on-subscribe for the miss case) is deleted, not implemented; §4 now records what topology gave instead.

**What it tightens, and the fact that this is the point.** The old arm was id-as-capability: any holder of `sub:subscribe` who knew a session id read that session's frames off the root ring, opening channel snapshot included, whoever owned it. Cross-principal visibility is now structural silence, and restoring it deliberately is a `node`-scope grant through `attachableNodes` (ADR 102 ship order 4), never a filter. Recorded as a README gap in `@agentick/gateway`, not as a regression.

**One leak the live path could not close, found by the cross-principal test.** `resolveChannelSnapshots` is a READ of session state (`app.getSession(id).channelSnapshot(...)`) rather than a bus delivery, so the bus tree does not carve it: alice subscribing to bob's id still received bob's board as her opening frame. It now asks the attachable paths the same reachability question once, at subscribe (`withinAttachment` — is the session's node inside a path the caller may attach to), which is where ADR 102 puts every authorization decision. `session-tree`'s splice is left alone: its live stream is still id-named, and a snapshot stricter than the stream it opens is incoherent.

**`session-tree` is UNCHANGED and stays live-only** (root ring + `onlyInTree`): subtree membership changes on every spawn, so it cannot be a bus-side query. It is now the only scope that answers `SessionNotFound`, and the only one not carved by principal — both stated in the gateway README's gap list.

**Two tests were rewritten rather than deleted, because their reason survived the mechanism.** `session-resume-e2e.spec.ts`'s "observation does NOT remount" asserted the refusal; it now asserts admission plus an untouched registry, which is the claim it always meant. `transport-websocket`'s "a session subscription survives a gateway whose session registry is EMPTY" used to heal by re-asking until the session existed and opening on a spliced snapshot; the resubscribe now succeeds immediately and the stream is open across the whole remount, so it receives the channel being BUILT — the empty opening snapshot, then the delta setting the knob. Strictly more information; the assertion follows the frames.

**Surface trims (ratified in review).** `createApp({ scopeNodes })` is marked framework-internal and removed from the app README's options table — it is the gateway's injection channel, and an adopter never constructs a `ScopeNodeRegistry`. Both READMEs were repositioned: the gateway is where topology is configured (one function keyed by principal, or nothing — the defaults already isolate), and the app-level `sessionNode` is the mechanism it configures through, used directly only by an embedded host with no gateway.

Four new pins in `subscription-principal-isolation.spec.ts` (draft-flow race: mint → subscribe → silence → send → frames on the pre-existing subscription; cross-principal silence, both ways; evicted → quiet → resume-by-send; unauthenticated still reads the root) plus a session-scope node-lease teardown pin in `subscription-teardown.spec.ts`. Discrimination checked: with the arm reverted to stage-2 behavior, three of the four fail (the unauthenticated one is behavior-preserving by design).

Gates: root `npx vitest run` 745 files / 7376 tests green, 0 failed, no flakes (both known-flaky suites passed in the same run); `pnpm typecheck` 111/111; oxfmt on every touched file; NUL byte-scan clean over all 11 changed source/doc files.

**2026-08-23 — ADR 102 stage 2: attachment IS the isolation; `onlyOwnedBy` is deleted (uncommitted, `feat/v2`).**

`gateway`/`app` subscriptions no longer inspect envelopes. `sub/subscribe` resolves an ATTACHMENT SET once — `attachableNodesFor(identity)`'s nodes with the caller's full query, plus the root restricted to a host-owned allowlist (`CONTROL_PLANE_NAMES = [GATEWAY_CAPABILITIES_CHANGED]`, in `subscriptions-extension.ts`) — and merges them onto one stream. Selection is by topic, never by whose frame arrived. The allowlist deliberately does NOT reproduce the whole of what `onlyOwnedBy`'s no-sessionId branch admitted: that branch also passed app-level lifecycle across tenants, a metadata leak its own docstring named as a soft edge, and the allowlist closes it. The control-plane attachment carries the caller's own query minus its name, and is skipped entirely when the caller's `name` constraint excludes it — a subscriber that asked for model deltas must not be told the capability set moved. `session`/`session-tree` are UNCHANGED (stage 3), which works only because fan-in carries a node's frames to the root ring; that is pinned by a test, not asserted in prose.

**Stage 1's two couplings, both closed.** (A) The per-execution emission target: `withEmissionBus(bus, effect)` in `packages/runtime/src/substrate/emission-target.ts` — a FiberRef in the exact shape of the tier-4 `CallMiddlewareRef`, read by `OperationRunner.publish` / the new `publishTo` + `emissionBus` members and by `BaseHarness.emitLazy`/`emitDelta` (the two subscriber-index gates, which must probe the bus the frame will ACTUALLY land on or starve). The session scopes it around `loop.fx.runExecution` — ONE call site, covering send and resume, since `runExecutionCore` is the single loop-invocation path. This was chosen over threading a `bus` field through `RunExecutionInput` → `RenderTreeInput` → the model execute input: three protocol shape changes plus every double, and it would still have missed the loop's OWN `loop:run-execution` envelopes (emitted around the body, before any input-carried target could apply) and every future app-shared harness. Rejected earlier: a scope-routing bus facade for the shared spine — its `hasSubscriberFor(key)` receives no sessionId, so it would have had to OR over every live node per delta, an O(principals) walk on the hottest path in the system three days after an event-loop-saturation outage. Discrimination checked: with the wrap disabled, a node subscriber receives ZERO model deltas. (B) `sessionNodeFor` / `attachableNodesFor` / `attachScopeNode` are now on `GatewayHarnessProtocol`; `ScopeNodeLease` moved from `@agentick/runtime` to `@agentick/spec`'s `protocol/bus.ts` (spec cannot import runtime, and two shapes for one concept is worse than the move). Two structural doubles in `@agentick/transport` updated.

**The consistency trap, closed by making the gateway the single source.** The gateway's `sessionNode(auth)` and an app's `sessionNode(ctx)` were independently configurable and had to agree or subscribers attach where sessions never land. `createAppBody` now injects BOTH a resolver derived from `this.sessionNodeFor` and the gateway's OWN `ScopeNodeRegistry` when the app names no `sessionNode` — so the DEFAULT authenticated deployment gets `[principal]` placement and `[principal]` attachment aligned by construction, with an app-level resolver still winning. One guard: an app on its own bus is left topology-free, because the tree is rooted at the gateway's bus and relocating such an app's sessions into it would make `app.events()` blind to them. The mirror-image cost (a node subscriber not seeing that app) is logged as `TODO(scope-nodes-app-bus)` + a README gap.

The #299 principal-isolation spec is ported rather than replaced: every behavioral guarantee kept, the claims restated structurally (disjoint buses, no edge carries a sibling's traffic), and four added — the two principals' sessions landing on distinct node buses that ARE the ones the wire attaches to, one subscription carrying both its node frames and the control plane, a session-scope subscription still receiving LIVE frames off a node-resident session, and (in `subscription-teardown.spec.ts`, which owns that contract) an attachment set releasing every iterator plus the node lease. All four verified to fail when the mechanism is disabled.

Gates: root `npx vitest run` 745 files / 7371 tests green, 0 failed, no flakes; `pnpm typecheck` 111/111; oxfmt on every touched file; NUL byte-scan clean over all 18 changed files.
...
**Two replacements the deletion forced, both narrower than what they replace.** (1) `seed(values)` on knobs + state: `withX({ initial })` and `CreateSessionInput.initialKnobs`/`initialState` were the last live callers of `importSnapshot`, and they are a construction seed, not a restore. It UPSERTS where `importSnapshot` replaced wholesale — a latent bug, since the create-call seed runs AFTER the hydrate fan-out and replace semantics silently wiped everything the store had just restored. Pinned in both conformance suites and both store-backing specs. (2) `lastCompaction()` on `LogView` + the timeline protocol: projection provenance (`CompactStrategy.metadata`) had exactly one sink, the snapshot. Without a reader, a documented public field becomes a no-op. Also added: `PromptsHarness.record(name)`, the sync read of the serializable slice that `prompts:get` already projects to the wire.

**Accounting verified, not assumed.** Pre-sweep, `restore(snapshot)` reinstated `currentTick` / `usage` / `byModel` / `cost` from the blob. Post-sweep that data rides the durable `SessionRecord`, and `SessionRuntime.hydrate()` does adopt it — `usage` explicitly, `byModel`/`cost` through the `...persisted` spread (the fresh record omits both via `omitUndefined`, so nothing overwrites them). Proven end-to-end by a new test in `usage-cost.spec.tsx`: two turns across two harness instances sharing an id and a store, asserting the second turn's cost SUMS onto the first (121/13 tokens, 1000 micros). `currentTick` deliberately does NOT survive — it is execution-local and correctly excluded from the record.

**The fork law retired with the transport that motivated it.** ADR 93 said a child must skip genesis because it arrived via `restore(snapshot)`; under store transport a fork branches the scope and hydrates over the copy, so the skills/prompts genesis tests that policed it are deleted, and the surviving one is rescoped to what is still true (a second `hydrate` restamps live edits, which is why the caller owns when it runs).

**One sweep blocker, reported not hacked around: `LogView._persisted` stays.** Its stated blocker ("requires a transplant mechanism first") IS gone — store-layer `branch` is the fork transport. What blocks it now is different and was not visible from the manifest: `readPersisted()`, `trailingInput()` and `inputEntryCount()` are SYNC protocol reads of the whole log, and the session tick loop calls `inputEntryCount()` every tick. Deleting the mirror forces them async, converting a per-tick memory read into a per-tick full-log store read — a regression, not a cleanup. The real fix is the read-surface redesign data-layer §2.7 / Phase 6b already schedules (a maintained input counter + a bounded tail read). The `TODO(adr-93-d1b)` is rewritten in place to say so. The embedded log field on the snapshot — the actual god-blob — is gone with `TimelineHarnessSnapshot`.

Derivable-by-render harnesses (skills, prompts, subscriptions) lost their pairs outright; `TODO(phase-5-storify)` marks skills and prompts, where an IMPERATIVE `register()` is durable only if the definition configures `hydrate: hydrateFromStore()`. `ExecutionRunner.onPersist`/`onRestore` (kill-list item 7) was a no-op: `ExecutionRunner` does not exist in the v2 tree — CLAUDE.md still documents it as a v1 leftover.

Gates: root `npx vitest run` 736 files / 7288 tests green; `pnpm typecheck` 111/111; oxfmt on every touched file; NUL byte-scan clean over all 128 changed files; unfiltered dead-identifier greps 0 hits across `packages/`, READMEs, `skills/`, and the root instruction files (positive control: 111 files match `interface` under `packages/spec/src`). Residual hits confined to `docs/`, `plans/`, and `packages/timeline/CHANGELOG.md` — historical log and the dedicated docs pass. Tracking: #311.

**2026-08-22 — execution resume LANDED on `feat/v2`: crashed mid-turn executions re-drive under their original identity (#311 rider, `docs/proposals/v2/execution-resume.md`).**

Built on the checkpointing seams by two sessions in strict author/reviewer role-swap (slices 1-2 by the knowify session, reviewed here; slice 3 built here, reviewed there). The vertical: two-signal crash detection — the record's write-behind `running` is the CANDIDATE, the timeline turn boundary is AUTHORITATIVE (boundary present = only the idle-write was lost → clean record, no callback, never a re-drive); `markInterruptedRecord` + per-EXECUTION `resumeAttempts` budget (bump-or-reset by id); `onInterruptedExecution(records, attempt) → resume|drop` policy seam (default drop; throw = loud effective-drop, session stays live); `executionCursor` on the timeline harness (coordinates out, never entries); a PURE `runExecutionCore` extraction from sendBody (+21/−1) making resume the second caller of the ONE loop-invocation path — so ADR 53 busy-send rules compose UNCHANGED over a re-driven turn (plain sends steer it, structured queue); `resumeExecution` = the stripped send (own id, no append, no bump, `startTickIndex` seed, acceptance-only await); completion clears the interruption KEYED to the settling executionId in the shared settle. Review arc caught two real bugs pre-merge: the write-behind mark-clobber race (InMemory-masked; fixed with `flushRecordWrites` barrier + a discriminating async-store rig) and a vacuous first rig (delayed `put` when the view persists via `mutate` — instrumented both-ways discrimination is now the standard). E2E: crashed turn completes under its ORIGINAL executionId at tickIndex N+1. Follow-ups: `runExecutionResumeAcceptance` factory to certify DURABLE adapters (the race class was invisible in-memory — KnowifySessionStore must run the same kill rig); eager boot sweep stays application-composed (3 lines over `list({status:'running'})` + `resumeSession`) until three consumers exist. Commits `4436d05c4`→`967fc67cf`; full suite 7408/0.

**2026-08-22 — checkpointing COMPLETE on `feat/checkpointing` (#311): all phases landed, adversarial judge signed gate 7. Ready for the merge call.**

P3 (`4521f8939`): eviction = `session:snapshot` (flush barrier, rejected persist aborts, TOCTOU re-guarded) + `session:close(evicted)`; the `paged` map DELETED; `evictSession(id)` on the app protocol (triggers are callers); fork = snapshot + spawn + branch fan-out + restore; namespace slots gained an app-scoped-defaults arm (zero-config store continuity, adopter wins). P4 (`73066ed83`): the sweep — SnapshotCapable/SessionSnapshot/migration seam/wire verb/CompilerSnapshot/embedded timeline log all deleted, blob-only tests pruned not adapted; net −2,079. P5 (`2636389ee`): docs pass. **Adversarial judge pass** (independent agent, seven-invariant fidelity table): found ONE Critical — nothing deleted a store scope, so destroy leaked every session's partitions into the app-scoped default stores (the same retention relocated a fourth time) with an id-reuse resurrection corollary. Fix batch (`1949aa388`): `DropCapable.dropScope(ctx)` — the fourth leaf capability — fanned out by destroy across the subtree, records deleted LAST; non-live targets rebuilt through the shared resume path, terminal records included (close-then-destroy must not leak); plus de-vacuumed conformance (CheckpointProbe counters), restored negative pins (code/resources/prompts/skills — one `persist` from silent enrollment), `restore()` rejects in-flight (`SessionBusyError`), `omitUndefined` augment folds, spec-conformance de-namespaced (ADR 27). Judge re-verified and **signed**; its two ride-along gaps pinned in `3262b9832` (terminal-record divergence, drop-failure posture). Gates: full suite 7312 incl. the new pins / 0 failed; typecheck 111/111; branch net ≈ +900 LOC of which ~⅔ tests+docs. Follow-ups logged on #311: app-door usage pin, E12 ordering doc downgrade, per-record specVersion gap, doc-rot banners, `TODO(destroy-without-remount)`, `dropScopes` not on the protocol (third-party session harnesses carry no scope-free obligation), no interceptor seam on the drop step specifically.

**2026-08-22 — checkpointing P1+P2 landed (branch `feat/checkpointing`): CheckpointCapable + three harness migrations + BranchCapable.**

P1 (`97800b8fc`): `CheckpointCapable { persist, hydrate }` + ctx types in spec beside `SnapshotCapable`; the session fold gives CheckpointCapable precedence per bridge — persist fan-out BEFORE blob capture (rejected persist aborts the op), migrated bridges excluded from the blob, hydrate fan-out on restore. Seven pins in `checkpoint-fold.spec.ts`. P2 (`6e6e982d8`): timeline (persist IS the flush barrier; hydrate IS genesis — one store-read path, 12 LOC), knobs + state (scope-partitioned cells; one injected app-scoped store serves every session), `View.hydrate({replace})` in store. **Mid-phase finding (P1 and P2 agents converged independently): `session.fork()` was the blob's last live consumer** — every migrated harness silently dropped out of the fork path. Dual-write coexistence was proposed and rejected (it would keep the blob authoritative until a big-bang P4 flip); instead `BranchCapable { branch(ctx) }` landed in spec (harness copies the source scope onto its own scope at the store layer; the ADR 93 fork law retires with the blob transport — doc §5 amended). Blob-coupled test vehicles (spec-conformance session suite, kill-resume acceptance) migrated to live-handle reads; kill-resume's blob-transplant test became restore-idempotency. Gates: all P2-touched packages green (1543), workspace typecheck 111/111; six enumerated failures in packages/app are P3's opening work list (eviction/fork rewire — `paged` deletion). Tracking: #311.

**2026-08-20 — checkpointing proposal: the session is its stores (doc only, `docs/proposals/v2/checkpointing.md`).**

Motivated by production evidence from assistant-api (memory staircase: eviction parks the full `SessionSnapshot` — timeline god-blob included — in the app's unbounded in-memory `paged` map, `packages/app/src/harness.ts:955`). The doc is a delta on `data-layer-plan.md`: it promotes Phase 4 (the manifest sweep) and adds two decisions the plan left open — **eviction unification** (evict/restart/crash collapse to one recovery path, `build + restore(manifest)`; the `paged` map and `TODO(eviction-durable-snapshot)` are deleted) and **no export primitive** (transplant resolves at the store layer — shared-store move is free, fork is a scope copy; `SessionSnapshot`, `SnapshotCapable`, and the wire `session/snapshot` verb all die in the sweep). Leaf contract: `SnapshotCapable` (sync, value-returning — the signature the god-blob cascades from) → `CheckpointCapable { persist, hydrate }` (async hooks; flush + cursor report, no value crosses the seam). **Ratified same day** (judge pass with Ryan): spawns are process-bound (durable spawns = future opt-in, `TODO(durable-spawns)`); hook ctx is `{ sessionId, tick, storeCtx, signal }` with deliberately no `reason` field (hooks blind to trigger preserves the one-recovery-path guarantee); restore ordering fixed journal→derived→mount, no dependency declaration until a real counterexample (`TODO(hydrate-ordering)`). Day-2 production chart confirmed the diagnosis: memory staircase resumed from day 1's plateau (~20% → 22.9%), ruling out V8 high-water expansion. Doc is build-ready.

**2026-08-22 amendments** (same doc): `persist`/`hydrate` return **void** — the `ManifestEntry` return was the last trace of framework data-concern; each harness finds its own data by session scope (supersedes plan §2.5's cursor-map as a resume input, consistent with its resume-latest default; `SessionRecord.stores` re-scoped to observability-only-if-ever). Framework owns FSM + faithful hook dispatch + trigger mechanics, never the data. Triggers-are-callers invariant named; public `evictSession(id)` added to the app protocol as resume's missing symmetric verb. Failure semantics stated: rejected `persist` aborts the eviction (session stays live), rejected `hydrate` fails the resume. Eviction stays a composition of `session:snapshot` + `session:close(evicted)` — hooks ride the existing minted seams; `TODO(eviction-victim-policy)` trailhead for victim selection.
...
packages/spec/src/data/                                 ✓ wire types for Phase 3
  ...
  entries.ts            CacheBoundary, MessageEntry, MessageMetadata,
...
- **ROADMAP (2026-07-22, Ryan-approved — the standing task list; work top-down).**
  **Phase A — finish the approved queue:**
  ...
  machinery for — CacheBoundary — but never assert). + doc rule: time-varying content
