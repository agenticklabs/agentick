...
**Branch:** `feat/v2`
**Last updated:** 2026-07-24 — **README/website positioning + the three-audiences plan (docs only, no code).** Two new docs: `website-design.md` (positioning skeleton for README+website — thesis "every surface the model can touch, code can touch"; hero = one session, three drivers (author/drive-headless/serve); knobs demoted; claims ledger verified against source by 3 scout sweeps — notable: skills have NO run verb, `session.hooks` doesn't exist, cache-prefix claim BLOCKED on a missing prefix-stability test, Mode B gateway-MCP unwired, gates have no wire namespace) and `three-audiences-plan.md` (implementation spec, RATIFIED by Ryan: A gates+resources on the wire incl. new slim GatesHarness owning GatesController + client gates handle; B responseFormat on SendInput (trail-response-format-send) with wire-safe/live-schema dichotomy; C skills.run inline-default + NEW `session.fork()` primitive (snapshot→spawn-own-root→restore sugar; spawn=new image, fork=copy of self) for isolate:true; D per-harness model-tools convention — naming law `<harness-noun>_<verb>`, set_knob→knob_set sweep, ctx-slot rule (NO ctx.session), auto command→tool bridge REJECTED; E skill loaders (agentSkillsDirectory preset, references-as-RESOURCES composition, fromPackage; import-attributes deferred); F session.tools handle (sync View reads, dispatch verb, client twin + new tools:list wire read, session.dispatch REMOVED); G client handle parity law — skills/prompts/state wire-routed but handle-less today, resources declared-but-unrouted (SESSION_SURFACES omission), model handle deliberately held (authz)). Delegation protocol codified (scout-at-delegation-time, terrain-map briefs, anti-goals + stop rule). Next: PR A dispatch after its terrain scout.
...
- **ROADMAP (2026-07-22, Ryan-approved — the standing task list; work top-down).**
  **Phase A — finish the approved queue:**
  ...
  — exposed server + CORS _ = any website executes shell), no exec-from-URL patterns,
  ...
  **runClientHandleConformance** — a shared suite every package's /client handle
  ...
  §4)**: the React useOn_ hooks are OBSERVE-only projections (real command lifecycle,
