...
**2026-08-09 — a confirmation grant is observable and durable: `onConfirmationResolved` reports every ask, and the executor's grant set rides the session snapshot.**

**`always: true` meant "until this process forgets."** The gate honored it through an in-memory `Set<string>` on `ToolExecutorHarness` and told nobody, so the two halves an adopter needs were both missing: `confirmationPolicy` could READ a durable grants store but nothing could WRITE one, and a session restore silently dropped every grant the user had already issued — the user re-approving what they approved before the reload is the exact failure a confirmation gate exists to avoid. Both halves now exist. `onConfirmationResolved` is a flat construction option beside `confirmationPolicy` (so it reaches an adopter through `createApp({ toolExecutor })` with no new plumbing), fired after every ask that settles, forked so an observer writing to a store can neither delay nor fail the dispatch it reports on — a failure lands on the harness log surface at `warning` instead of vanishing. And the harness is now `SnapshotCapable`, so the grant set round-trips under `bridges.toolExecutor` — a slot the fold now refuses to let an extension bridge claim (`SnapshotSlotOccupied`), because a silent overwrite there loses the bridge's state on capture and feeds it a foreign shape on restore.

**The record is the spec's own shape, evolved rather than duplicated.** `ToolConfirmationResponse` — declared for the dormant `ToolConfirmationResolved` lifecycle event and used nowhere — became `ToolConfirmationResolution`: `approved: boolean` gave way to `outcome: "approved" | "denied" | "timeout" | "aborted"`, because an ask nobody answered is not a denial and an audit log that cannot tell them apart is lying; `toolName` / `sessionId` / `arguments` were added because a grant store needs a key and a scope, and that key is the CANONICAL declaration name — a grant keyed on the alias a caller happened to dispatch by covers nothing when the next call arrives under the real name. The dormant lifecycle wrappers that motivated the old shape (`ToolConfirmationRequest`, `ToolConfirmationRequested`, `ToolConfirmationResolved`, and the `tool-confirmation-resolved` stream event) are deleted: the observer is the only live path, and a type with zero emit sites is a promise the code does not keep. The grant and the approval record are written AFTER a `modifiedArguments` re-validation succeeds, so an edit that fails validation leaves neither behind — the approved call never ran. An observer that throws is logged at `warning` on the harness log surface rather than swallowed. The snapshot side needed no new mechanism but did need an honest one: the executor is a session-owned harness held OUTSIDE `bridges` (the `tools` slot is the render-time handler-resolver adapter), so `SessionHarness` folds it as a NAMED candidate — the precedent already set by `snapshotProviders()` for the `tool_call` channel. One `snapshotCandidates()` list now feeds capture, restore, and the channel scan, so the three cannot disagree about what participates; feature detection via `isSnapshotCapable` still does the actual selecting. A `fork()` is a spawn plus a restore, so a child now inherits its parent's grants — pinned by test, not inferred. Tests are written at the adopter's entry point (`createApp` / `createSession` / `session.snapshot()`), and the snapshot one was confirmed to FAIL with the fold disabled. Suites: spec + tool-executor + session + app, 142 files / 1497 green; workspace typecheck 106/106. Deliberately not done: handler state (`ctx.setState`) still does not survive a restore — it holds arbitrary live values, not a serializable record. Standing note for whoever adds a wire verb for restore: `session.restore` is deliberately NOT reachable over the wire today, and this change made the snapshot authorization-bearing (it carries standing grants) — a future `session/restore` verb must treat the snapshot payload as a privilege-escalation surface, not inert state.
