...
**Branch:** `feat/v2`
**Last updated:** 2026-07-24 (later) — **PR A of the three-audiences plan LANDED: gates + resources on the wire.** New slim `GatesHarness` (BaseHarness<"gates">) owns the ONE `GatesController` (ownership inversion in session-bridges; `bridges.gates` still resolves the controller so useGate/session.gates/notifyLifecycle untouched — controller.spec (12) + gate.spec (19) green UNMODIFIED); four `exposure:"wire"` commands gates:list/clear/defer/override (defer/override delegate to the per-gate GateHandle; missing gate → typed `GateNotFound`, new GatesError family in spec/errors); `GateOverrideAudit.origin` ("host"|"wire") stamped in-fiber via `getContext` — enabled by a ONE-LINE runtime fix: operation-runner now threads `scope.origin` into RuntimeContext (field was declared on EventScope but never populated; benefits every command handler). `SESSION_SURFACES` += gates AND resources (A1 was literally one line — resources' inbox auto-binds via the BaseHarness ctor and its commands were already wire-exposed but unrouted). ADR-87 client handle `client.session.gates` (RPC fire-and-refetch — NO gates-state channel yet, documented gap rides the store fan-out) + type-only wire-augment + bundle import. e2e: gates-e2e + resources-e2e in transport-in-process (copied tasks-cancel-e2e, the structural twin template; deny-by-default asserted). KEY SCOUT FINDINGS pinned in three-audiences-plan.md: inbox binding is AUTOMATIC (BaseHarness ctor; registerNamespace is unrelated); state's commands omit exposure → default "addressable" so state is NOT actually wire-reachable despite being in SESSION_SURFACES (G fixes); withState's extension.ts is NOT the live install path. Value-cell stratification trailhead recorded (gates-on-knobs ruled deliberate composition; TODO(value-cell) at the GateKnobs seam). Delegation protocol (scout-at-delegation-time, terrain-map briefs, anti-goals+stop rule) used for the first time — the implementer hit the origin seam, took the principled fix, and REPORTED the deviation as briefed. Suite 4713 → 4737 (+24: 13 gates, 11 transport-in-process); typecheck 154/154 --force; no-TLA green. Plan docs landed at 81350ebc; PR A commit follows. Next: PR B (responseFormat on SendInput) after its scout.
