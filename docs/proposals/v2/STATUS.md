...
**Branch:** `feat/v2-compiler-phase-3`
**Last updated:** 2026-06-27 (ADR 39 Phase 3 Step 3b shipped) — **Declaration intrinsics land in the compiler-react walker.** `<tool>` / `<mcp>` / `<resource>` / `<output>` / `<model>` produce runtime declarations + generation-time config from compiler-next helpers (`toolDeclaration` / `mcpDeclaration` / `resourceDeclaration` / `outputDeclaration` / `modelConfig`). `WalkResult` grew `tools?` / `mcps?` / `resources?` / `outputs?` arrays; `dispatch-declarations.ts` is a new file in compiler-react architecturally distinct from `dispatch-block.ts` (different IR targets — `tree.declarations.*` + `tree.config` + `tree.providerOptions` vs `tree.context.entries` + `tree.content`). Walker routes declaration tags BEFORE block-mode dispatch so `<tool>` doesn't fall through to the role-shorthand path it used to incorrectly claim. Diagnostic codes: `tool-missing-name`, `tool-missing-input-schema`, `mcp-missing-fields`. **The `<tool>` tag now consistently means tool DECLARATION** — tool-result messages must use `<message role="tool">` explicitly (callers were all using declaration semantics; no regression in practice). 22 new declaration tests (`declarations.spec.tsx`); 186 files / 2053 tests green; workspace typecheck clean modulo pre-existing v2-real drift.

**Step 3b honors the refined Path B framing:** only the uppercase `<Tool>` reactive component (handler-closure-capture via React hooks) actually needs `ToolBridge`. The lowercase `<tool>` and the other 4 lowercase intrinsics are purely declarative; they belong to the IR builder (compiler). If MCP/Resource/Output/Model ever grow reactive concerns later, the pattern is documented at the top of `dispatch-declarations.ts`: introduce an analogous `XBridge` mirroring `ToolBridge` (handler-register), but the declaration still flows through this walker dispatch.

**Previously, 2026-06-26 (ADR 39 Phase 3 Step 3a shipped) — Compiler-react walker grows formatter scope + diagnostic channel.** `WalkResult` broadens from `{entries, blocks}` to `{entries, blocks, diagnostics?, specConfig?, providerOptions?}`. New `<format formatter={ref} purpose?>` intrinsic in compiler-next + compiler-react: passthrough that stamps `renderedWith` on descendant section / message entries via a parent-link `WalkScope` (mirrors reconciler-next's `HostScope` semantics — consolidation deferred to Step 3d). Diagnostic surface lights up the previously-silent drop paths (media missing source, system_event missing event, user_action missing action, state_change missing entity, custom missing fields, malformed `<format>` props). `sectionEntry()` / `messageEntry()` helpers in compiler-next accept `renderedWith` so the stamp threads through unchanged. 12 new format-scope + diagnostic tests; 2029 v2 tests green; workspace typecheck clean modulo pre-existing v2-real handler-signature drift.

**Step 3a unblocks Step 3b** (declareInRender on reactive bridges → 3c port reactive components → 3d differential gate where reconciler-react delegates JSX→IR to compiler-react → 3e delete `reconciler/collect/`). Path B locked: reactive intrinsics (`<Tool>`, `<MCP>`, `<Resource>`, `<Output>`, `<Model>`) become React components in `reconciler-react/components/` that emit declarations via HookBridges at render time; the static walker stays principled.

**Previously, 2026-06-26 (ADR 39 drafted)** — **ADR 39 — JSX template walker: one IR, two evaluation contexts.** Proposed framing for the next architectural lift: pull intrinsic-component semantics into a shared dispatch table, ship a static walker (`@agentick/jsx-template-next`) that consumes the same table as reconciler-react-next, and unlock prompts (#121), resources (#123), tool descriptions, MCP server prompts/resources (#171), and snapshot-testable templates from one primitive. Async `render(Template, props): Promise<string>` with compile-until-stable (`useData` suspends via thrown-Promise, walker awaits + retries). Reconciler-agnostic IR — Angular / Solid / custom AST plug into the same dispatch table. Three-phase rollout: parallel static walker → adopt downstream → optional differential-gated refactor of reconciler-react-next. Estimated 6–11 days end-to-end, no risk to existing 1925-test suite (parallel by default).
