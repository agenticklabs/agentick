...
**Branch:** `feat/v2`
**Last updated:** 2026-07-24 (later-2) — **PR B LANDED (descoped to canonical plumbing) + B2 SPEC RATIFIED (terminal-tool strategy).** Ryan challenged `output` mid-flight (multi-tick structured output: responseFormat-every-tick strangles tool-tick narration, is provider-partial (anthropic+ai-sdk DROP it), and decouples done-from-shaped) → B2 designed and ratified: terminal tool whose inputSchema IS the output schema; call = completion event; loop-side detection by name → stop; binds via EXISTING execution-scoped SendInput.tools (no forks, no prompt injection — the tool IS the instruction); tree-tier via the ALREADY-SEEDED-but-unconsumed OutputDeclaration (declarations.ts:547); strategy seam auto tool-vs-responseFormat; prefill-cache rule: execution-scoped bindings serialize at the TAIL after the stable-tools cache breakpoint (Anthropic prefix survives; OpenAI/Google one-off bust bounded+documented); canonical toolChoice RATIFIED+pulled forward ("auto"|"none"|"required"|{tool} — 4-adapter mapping table in plan §B2); guarantees contract: mechanism=CI-scripted, provider-contract=adapter units, behavioral compliance=@agentick/eval NUMBERS never CI asserts; forced tool_choice is the HARD rung. B as landed: SendInput/SessionSendParams/RunExecutionInput/TickInput.responseFormat threading with loop overlay (send-level spreads LAST over tree config + modelDecl.parameters), parseJsonWithSchema extracted to spec/data/standard-schema (generateObject refactored on it, its spec green UNMODIFIED), TODO markers at BOTH drop sites (anthropic + ai-sdk), SteerCannotCarryResponseFormat (steer-JOIN keeps only messages — a steer carrying responseFormat rejects, followUp works). output/SendResult.data DESCOPED into B2a (return with the correct strategy). PROCESS INCIDENT (recorded): the descope amendment sat in the agent's inbox while it built the FULL brief and reported; the report-request nudge resumed it and it then applied the descope — Ryan's crossed "land as-built" was overtaken; ruling: land descoped (matches ratified spec; less interim debt). My mid-surgery typecheck --force failed transiently (agent editing live) — pipe-masking hid the exit code initially; unmasked re-run on the quiesced tree: 154/154. Lesson: quiesce before gating; never pipe a gate. Suite 4737 → 4750/0; no-TLA green. Next: toolChoice PR (sequential — file overlap with B) + B2a scout in parallel; then B2a (output/data/terminal-tool/wrap-up tick/eval suite); C rides B2a.
