...
**Churn stance:** churn is _invited_ where it reaches a brighter design. The bar is
...
- **`journal` ≠ `TimelineStore`.** TimelineStore is a journal _in spirit_
...
in-memory array is the source of truth and the store _lags_. `applyAppend` pushes
...
it: _the log IS the store; hold only the bounded projection._
...
keeps a _separate_ `hydrated` promise because `ready` is a readonly BaseHarness
...
interface SessionManifest {
  ...
  readonly stores: Readonly<
    Record<
      string /*harness, incl. "session"*/,
      {
        readonly scope: string;
        readonly key: string;
        readonly backend: string;
        readonly cursor: Cursor; // seq / version / offset
      }
    >
  >;
...
> **Bootstrapping note:** the manifest is the _index_ you read to discover a
...
- **credentials proves the wire surface is optional** — its surface is _empty_
...
### 2.7 Timeline: the log is the store; compaction is an overlay event _(RESOLVED — de-opinionated)_

**Correction (Ryan, on my over-reach):** an earlier draft here mandated a
_bounded_ projection + write policy at the framework level. That is **impl
...
- **Contract (framework):** the entry store is a **`LogStore`** — append + a
  cursored read (+ ordering). The harness _appends to it_ and _reads what it needs
  to compile_ through that contract. Nothing more.
...
- The one non-opinion cleanup: `_persisted` is a full log mirror living **in the
  harness** (storage in the logic layer). Move log storage _into the store_ (the
  store it moves into may still hold a full array) — separation of concerns, a
  _harness_ refactor separable from the store-contract run.
...
raw entries it covers **stay** in the log; the compaction event is an _overlay_
...
- **Deterministic** — the non-determinism happened once, when the compaction was
  _created_ (the LLM); the summary is **durable on the event**. Replay is a
  deterministic fold → **§6-E dissolves**, no re-running the model, no
  projection-cache store.
- **Framework knows the _structure_ (range + summary) to fold for the MODEL; the
  app owns the _content_ (summary text, strategy) + _UI rendering_** (full history,
...
column that matters for _how_ is the **projection strategy** — eager write-through
...
already converged on the primitives; the playbook is mostly _conform + verify_, not
_invent_. Reuse targets (do NOT reinvent — grep first):
...
- [ ] **P0 — Classify.** Discriminator: does the state have a lifecycle
      _independent of the session snapshot_? If no and it's tree-derivable →
...
- [ ] **P3 — Align the query.** The query = **`EventScope`-based scope** (+ the
      ...
      predicates**. It identifies _scope, range, order, basic params_ and **leaves
...
- [ ] **P5 — Wire to BaseHarness + choose the projection.** Mount on the `store?`
      ...
      **render-read** (a hook reads it during the sync render pass), and (2) a **sync
      ...
      _synchronously, un-awaited_ (`reconciler-harness.ts:926`), so a `SnapshotCapable`
      ...
      projection IS needed, is _population_: **EAGER write-through** =
      ...
      **async-only, non-`SnapshotCapable`** harness (credentials) needs **NO
      ...
      _pure-mirror_ collection = `CollectionProjection` (knobs, state); eager
      _augmented_ collection = hand-rolled, cache holds the record + non-persisted
      extras (tasks: `live` = record + fibers/abort); eager _augmented single-record_
      = hand-rolled, one record + ephemeral extras (session: `SessionRuntime` = record + `currentTick`/listeners); _lazy_ = `useData` (prompts/skills/resources); _none_
      = async-only (credentials); _bounded fold_ (timeline). Augmented/single-record
      ...
      (non-negotiable): the durable backing is a `Store` (`CollectionStore`/`LogStore` + a `createXStore` factory); the live sync working copy is a PROJECTION — name
...
- [ ] **P6 — Client surface.** Framework-minimum: `enumerate` + `page` wire methods + one change channel. App extends freely. `⊇` pass-through over-fetch.
...
- [ ] **P8 — Conformance + parity.** Extend `runStoreConformance` with the entity's
      cases; every prior behavior maps or is a _documented_ drop (§7). Type-enforce
...
- **All state is store-derived AND store-persisted — the store is the single
  source of truth.** Every harness/layer's state _comes from_ the store (read /
  hydrate) and is _stored to_ the store (write / persist). No exceptions —
  ...
  genuinely _tree-derived transient_ state — a `<Resource>` tree-mount is NOT
  ...
  truth" governs the _durable_ half; it does not conscript state whose home is the
  tree. This is why such a harness stays NOT `SnapshotCapable` — its store is a
  _durable-source backing_, not a hibernate snapshot.
...
- **Substrates are store-backed** (confirmed): bus + journal = `EventLog`; inbox =
  `MessageInbox`. The playbook _verifies_ them, doesn't rebuild them.
...
1. **The shared conformance is deliberately THIN — only 3 archetype-agnostic
   ...
   store-specific via a `cases` hook. This _confirms_ "no `Store` supertype" — even
...
4. **`onChange` is for EXTERNAL / cross-consumer observation, NOT self-caused
   ...
   observes changes to a **shared** store that a consumer did _not_ cause
...
### Phase 0 — Store convention + generics + shared conformance _(pure additive, zero behavior change)_
...
- _Gate:_ all existing store + harness conformance green, workspace typecheck, no behavior delta.
...
- _Gate:_ kill/resume acceptance (`runKillResumeAcceptance`), timeline two-tier conformance, tasks conformance.
...
- _Gate:_ pure stores unaffected; derived store passes the same collection conformance + a replay/`asOf` test.

### Phase 3 — knobs ✓ + state become store-backed _(first storification; additive)_
...
- _Gate:_ channel snapshot+delta parity; kill/resume with knobs/state; sync reads
  unchanged; **net-additive is expected** (store + projection beside the notify
  seam) — the deletion is Phase 4, not here.

### Phase 4 — Manifest + `SessionStore` + `SessionHarness.restore` _(the un-built Step 6)_
...
- _Gate:_ kill/resume acceptance passes via manifest for all migrated harnesses; skew-window test; heterogeneous (some embedded, some manifest) resume.
...
- _Gate:_ per harness — store round-trip + loader population + (resources) catalog
  projection parity + template resolution; existing conformance green.

### Phase 6a — Timeline **entry-log store** _(the clean, opinion-free run — next)_
...
- Does NOT touch the harness `_persisted`/`_projection`/write-behind (impl
  opinion, left as-is) — this is _just the store archetype_, like every prior run.
- _Gate:_ existing timeline store-conformance green via the shared skeleton; `seq`
  contract preserved; no behavior delta.

### Phase 6b — Compaction overlay + fold _(additive, follows 6a)_
...
- _Gate:_ fold determinism (same log ⇒ same projection); full-vs-compacted render
...
- _Gate:_ language-agnostic wire (no TS-inference dependency); ui-next hooks against a real gateway; over-fetch surplus reaches UI, framework ignores it.
...
- **E1 — Store scoping (session vs app-singleton).** Timeline is per-session
  ...
  serve both; manifest references `(scope, key)`. _Do not_ force timeline into an
...
- **E2 — No atomic resume without event sourcing (cost #1).** Independent stores
  ...
  _stated guarantee_, not a bug. Flush barrier narrows the window; journal-derived
...
- **E5 — Non-deterministic timeline projection. RESOLVED (§2.7 / Phase 6b) —
  ...
  compaction is _created_; its `summary` is stored **on the compaction event** in
...
- **E6 — knobs values vs descriptors.** Store holds **values**; descriptors stay
  ...
  exactly this.
- **E7 — credentials empty wire surface.** A store that projects _nothing_ to the
...
SessionStoreQuery>`** (keyed by `id`, **app-singleton** like `TaskStore`; port +
  ...
  `list(query)` (by `appId`/`status`/`parentSessionId`/recent-`updatedAt`) _is_ the
  ...
  ownership:
  - _identity/lifecycle (framework-owned):_ `id`, `createdAt`, `updatedAt`,
    `status`, `parentSessionId?` (spawn ancestry → session tree), `appId?`.
  - _runtime accounting (framework-owned), hierarchy-aware (session → execution →
    tick):_ `currentExecutionId?` + `executionCount` (NOT `currentTick` — tick is
  ...
  - _descriptive (app-owned SLOTS — framework STORES, never POPULATES):_ `title?` /
...
- **E14 — `onChange` semantics across backends.** In-memory fires synchronously;
  ...
  change _channel_ (client-facing) is fed by the harness's own writes when the
...
- **E17 — Notify seam duplicated across harnesses. PARKED (teed up, do after the
  ...
  `TODO(notify-seam): promote to protocol` trailhead. The _primitives_ already
...
- **E18 — Async-aware `captureBridgeSnapshots` unlocks projection-free stores
  ...
  is _forced_ to keep an **eager** sync projection to serve it — even the ones that
...
- **§6-F — Consistency default. RESOLVED (event sourcing baked in).** Event
  ...
  for that harness. The framework guarantees the _capability_ is universal; the
  _behavior_ is opt-in. Capability-not-opinion.
...
- **No god snapshot.** The manifest references; stores hold. (This _is_ the plan.)
...
_Companion: [`data-layer-working-notes.md`](./data-layer-working-notes.md) (the
...
(sync engines — the bright line)._
