...
- Every `yes` still requires a **grant** (deny-by-default): shipping slice 5 exposes nothing to any principal until `staticAuthorizer({ grants })` says so. The `wire?` column decides what is _grantable_, not what is _granted_.
